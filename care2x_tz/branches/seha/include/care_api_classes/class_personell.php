...
class Personell extends Core {
	...
	*/
	...
	/*
	 * Table for users to care2x
	 */
	var $tbl_users='care_users';
	/**
	...
	*/
	function getPersonellInfo($nr){
		global $db;
		$sql="SELECT ps.*,p.*,
			c.funk1,c.funk2,c.inphone1,c.inphone2,c.inphone3
				FROM $this->tb AS ps,
	...
	*/
	function loadPersonellData($nr=0){
		...
		$this->sql="SELECT ps.nr, ps.short_id, ps.pid, ps.job_type_nr, j.name AS job_function_title, ps.date_join, ps.date_exit,
				    ps.contract_class, ps.contract_start, ps.contract_end, ps.is_discharged, ps.pay_class, ps.pay_class_sub,
				    ps.local_premium_id, ps.tax_account_nr, ps.ir_code, ps.nr_workday, ps.nr_weekhour, ps.nr_vacation_day, ps.multiple_employer,
				    ps.nr_dependent, ps.status, ps.history, ps.modify_id, ps.modify_time, ps.create_id, ps.create_time, p.title, p.name_last,
