...
<table border=0 bgcolor="#000000" cellpadding=1 cellspacing=0>
  <tr>
   ...
   <tr>
     ...
     <?php 	include_once($root_path.'include/care_api_classes/class_person.php');
				$alergic= new Person;
				$mypid = new person;
				$pid = $mypid->GetPidFromEncounter($pn);
				$alergic->LookForAlergy($pid);
				//echo 'ooo'.$pn;
				?>
	</div>
...
echo '
  ...
  <?php echo $LDRequestingDoc ?>:
		<input type="text" name="send_doctor" size=40 maxlength=40 value="<?php ($stored_request['send_doctor'])? print $stored_request['send_doctor'] : print $HTTP_SESSION_VARS['sess_user_name']; // if($edit_form || $read_form) echo $stored_request['send_doctor'] ?>"></div><br>
		</td>
    </tr>
	<tr bgcolor="<?php echo $bgc1 ?>">
