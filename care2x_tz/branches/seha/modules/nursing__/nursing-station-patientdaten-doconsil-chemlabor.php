	...
	*/
	...
	while(list($x,$v)=each($HTTP_POST_VARS)){
    	if((substr($x,0,1)=='_')&&($HTTP_POST_VARS[$x]==1)){
    		...
    		//echo $buffer."<br>";
    		array_push($param_array,substr($buffer,0,-1));
    		//echo $x;
	    	if($paramlist==''){
				$paramlist=$x.'=1';
			}else{
...
?>
<br>
<div style="width:740px; padding:4px 0px 4px 0px; float:left;">
<?php
		$pid = $alergic->GetPidFromEncounter($_GET['pn']);
...
<!-- Middle block of first row -->
      <td bgcolor="<?php echo $bgc1 ?>">
		 <table border=1 cellpadding=1 bgcolor="">
     <tr>
       <td>
...
echo '
					...
					<font color="purple">'.$LDAdmNr.'
						<font color="#ffffee" class="vi_data"><b>'.$pid.'
					</td>
					<td width="33%">
