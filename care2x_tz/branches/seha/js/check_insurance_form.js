...
function submit_form(reference, t, session, todo) {
  ...
  var parameters="reference="+reference.name;
	switch(todo)
	{
		...
		case "company_id":
	  	try
	  	{
			  ...
			  if (document.forms[0].elements['company_id'].options[i].selected == true)
			    	parameters += "&company_id="+document.forms[0].elements['company_id'].options[i].value;
		  }
		  catch(E){}
		  break;
	}
	try
	{
		parameters+="&company_id="+document.forms[0].elements['company_id'].value;
