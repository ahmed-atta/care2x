<?php
	...
	if($ttf_ok){
		...
		if(stristr('AB',$result['blood_group'])){
  		...
  		#  Full encounter nr
    	ImageString($label,4,170,2,'PID:'.$pid,$eblack);
    	ImageString($label,5,2,2,'FileNo:'.$fileno,$eblack);
		# Encounter admission date
    	ImageString($label,2,2,18,$result['pdate'],$eblack);
		# Family name, first name
    	ImageString($label,5,2,40,'Names: '.$result['name_last'].', '.$result['name_first'],$eblack);
		# Date of birth
    	ImageString($label,3,2,55,'Birth date:'.$result['date_birth'],$eblack);
		...
		# Address, zip, city/town name
    	ImageString($label,4,10,90,'Zip:'.strtoupper($result['addr_zip']).' '.$result['citytown_name'],$eblack);
		# Sex
    	ImageString($label,4,218,55,'Sex:'.strtoupper($result['sex']),$eblack);
		# Family name, repeat print
    	ImageString($label,5,30,125,'',$eblack);
		# Insurance co name
    	ImageString($label,4,10,140,$ins_obj->getFirmName($result['insurance_firm_id']),$eblack);
		# Location
    	ImageString($label,3,2,160,'Dept: '.$locstr,$eblack);
    	# Alergy
    	ImageString($label,4,10,130,'Alergy: '.$spec[0],$eblack);
		#Blood group
		ImageString($label,4,10,110,'Blood Group:'.$spec[1].' '.$spec[2],$eblack);
	}
