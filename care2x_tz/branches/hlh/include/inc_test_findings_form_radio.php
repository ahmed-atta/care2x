...
*/
		  ...
		  $sql='SELECT doctor_id FROM care_test_findings_radio WHERE batch_nr='.$batch_nr;
$doctor=$db->Execute($sql);
$sendingdoctor=$doctor->FetchRow()
		  		  ?>
            </div></td>
			<td align="right"><div class=fva2_ml10><font color="#000099">
									<font
								size=1 face="arial"> <?php echo $LDReportingDoc ?>:
<?php
...
while ($doctor_list=$doctors->FetchRow()) {
	if (($doctor_list[0].' '.$doctor_list[1])==$sendingdoctor['doctor_id']) {
//		echo '<option selected value="'.$doctor_list[0].' '.$doctor_list[1].'">'.
		echo $doctor_list[0].' '.$doctor_list[1];;
	} else {
