...
class weberp {
	...
	var $weberpcalls = array (
		...
		'getStockCatProperty'=>"weberp.xmlrpc_GetStockCatProperty"
	);
	...
	function weberp() {
		...
		$this->DebugLevel = 2;
		$this->ServerURL = "http://weberp-test.haydom.co.tz/api/api_xml-rpc.php";


			$this->user = php_xmlrpc_encode("care2x");
			$this->password = php_xmlrpc_encode("clement");
	...
	function getStockCatProperty($Property, $StockID) {
		$details[0]=$Property;
		$details[1]=$StockID;
		$propertydetails=$this->transfer($details, $this->weberpcalls['getStockCatProperty']);
		if ($propertydetails[0]==0) {
			return $propertydetails[1];
		} else {
			return -1;
		}
	}
