...
class Person extends Core {
	...
	*/
	...
	function GetNewSelianFileNumber(){
		...
		// Patch for db where the pid does not start with the predefined init
		//$this->sql="SELECT pid FROM $this->tb_person WHERE pid=$init_nr";
		$this->sql="SELECT max(selian_pid) as maximum FROM $this->tb_person WHERE create_time>'2009-12-31 11:40:12'";
		if($this->result=$db->Execute($this->sql)){
			if($this->row=$this->result->FetchRow()){
				if ($this->row['maximum']=='') {
					$maximum=0;
				} else {
					$maximum=$this->row['maximum'];
				}
				$i=0;
				while ($this->SelianFileExists($maximum+$i)) {
					$i++;
				}
				return ($maximum+$i);
			} else { return false; }
