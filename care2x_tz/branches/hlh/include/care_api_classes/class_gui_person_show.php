...
class GuiPersonShow extends Core {
...
/*
	...
	*/
	function display($pid=0){
		...
		$this->sql="select name from care_tz_religion where nr=".$religion;
		$this->result = $db->Execute ( $this->sql );
		$row = $this->result->FetchRow ();
		if (!$GLOBAL_CONFIG['person_religion_hide']&&$religion){
			$this->smarty->assign('sReligion',$this->createTR($LDReligion,$row['name'],2));
		}
