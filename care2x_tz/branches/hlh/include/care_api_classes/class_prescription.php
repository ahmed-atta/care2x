...
class Prescription extends Core {
	...
	*/
	...
	function getDrugList($class, $is_enabled ) {
		...
		$debug = FALSE;
		($debug)?$db->debug=TRUE:$db->debug=FALSE;
		$this->sql='SELECT item_id as drug_id, partcode as stockid, item_description as description FROM '.
			...
			if ($is_transmit_to_weberp_enable==1 and $class=='drug_list') {
...
//TODO: check on enabled drugsandservices item!
	...
	function GetNameOfItem($item_number) {
		...
		$debug=FALSE;
		($debug)?$db->debug=TRUE:$db->debug=FALSE;
		$this->sql="SELECT item_description as description FROM $this->tb_drugsandservices WHERE partcode = '".
			$item_number."' ";
		if ($this->result=$db->Execute($this->sql)) {
			if ($this->result->RecordCount()) {
				$this->item_array = $this->result->GetArray();
