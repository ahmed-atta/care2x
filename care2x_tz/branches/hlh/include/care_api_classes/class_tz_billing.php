...
class Bill extends Encounter {
	...
	function getPrice($item_id, $pricelist_no=1) {
		global $db;
		$this->debug=FALSE;
...
//		echo $this->sql;
	...
	function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no){
		...
		INSERT INTO $this->tbl_bill_elements " .
				...
				"		prescriptions_nr)" .
				"
		VALUES (".$bill_number.", '".time()."', 0, 1, 0, 0, 0, '".$row['dosage']."', '".$row['dosage']."', ".$row['times_per_day'].",". $row['days'].", ".$price.", '".$row['article']."', '".$row['notes']."', ".$row['article_item_number'].", '".$insurance."', '".$contract['id']."', '".$row['nr']."')";
	...
	function StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $bill_number, $insurance){
		global $db, $root_path;
		$this->debug=false;;
		if ($this->debug) echo "<b>class_tz_billing::StoreLaboratoryItemToBill(pid: $pid, batch_nr: $batch_nr, bill_number: $bill_number, insurance: $insurance)</b><br>";
		($this->debug) ? $db->debug=TRUE : $db->debug=FALSE;
		// do we have pending issues of prescriptions?
		// read all items out of the prescription table
	...
	$contract = $insurance_tz->CheckForValidContract($pid);
          ...
          $this->chemlab_amount = 1;
           ...
           $this->sql ="INSERT INTO $this->tbl_bill_elements (nr, date_change, is_labtest, is_radio_test, is_medicine, is_comment, is_paid, amount, amount_doc, times_per_day, days, price, balanced_insurance, insurance_id, description, notes, item_number, prescriptions_nr)
            VALUES (".$bill_number.", '".time()."',1,0,0,0,0, ".$this->chemlab_amount.",".$this->chemlab_amount.",0,0,'".$this->GetPriceofItem('item_id')."','".$insurance."',".$contract['id'].",'".$drg_obj->getItemDescription['item_id']."','".$row['notes']."','".$row['item_id']."', ".$row['prescrip_nr'].")";
...
//			VALUES (".$bill_number.",".time().",1,0,".$this->chemlab_amount.",'".$row['price']."','".$insurance."','".$contract['id']."','".$row['name']."')";
	...
	function StoreRadiologyItemToBill($pid, $batch_nr, $bill_number, $insurance){
		global $db, $root_path;
		$this->debug=false;
		($this->debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	$contract = $insurance_tz->CheckForValidContract($pid);
          ...
          $this->radio_amount = 1;
           ...
           $this->sql ="INSERT INTO $this->tbl_bill_elements (nr, date_change, is_labtest, is_medicine, is_radio_test, amount,
            times_per_day, days, price, description, notes, item_number, balanced_insurance, insurance_id,
            prescriptions_nr) VALUES (".$bill_number.",'".time()."',0,0,1,".$this->radio_amount.",0,0,
            '".$drg_obj->getPrice($row['test_request'])."','".$drg_obj->getItemDescription($row['test_request'])."',
            '".$row['notes']."','".$row['batch_nr']."','".$insurance."','".$contract['id']."', ".$row['batch_nr'].")";
...
//			VALUES (".$bill_number.",".time().",1,0,".$this->chemlab_amount.",'".$row['price']."','".$insurance."','".$contract['id']."','".$row['name']."')";
		...
		$this->sql = "UPDATE $this->tbl_rad_requests SET bill_number='".$bill_number."', bill_status='pending' WHERE nr= ".$row['batch_nr'];
...
//			if($bill_elems_row['is_radio_test']==1)
		  ...
		  if (empty($this->price)) $this->price="0,00";
			...
			echo '<tr>';
			echo '<td>'.$LDPrice.'</td>';
			echo '<td style="text-align: right;">'.$this->price.'</td>';
			echo '</tr>';
			...
			echo '<tr>';
			echo '<td>'.$LDPaidbyInsurance.'</td>';
			echo '<td style="text-align: right;">'.number_format($insurance_used,2,'.',',').'</td>';
			echo '</tr>';
			...
			echo '<tr>';
			echo '<td>'.$LDpartSum.'</td>';
			echo '<td style="text-align: right;"><b>'.number_format($mySum,2,'.',',').'<b></td>';
			echo '</tr>';
	 ...
	 $printout=func_get_arg (3);
		...
		if ($billnumbers) {
			while($bills=$billnumbers->FetchRow())
			{
				...
				echo '<tr>
								<tr><td ><span style="font-family:&quot;20 cpi&quot;"><font size="4">'.$LDSummary.'</font></span></span></td></tr>
								<td>
									<table border="0" >
										<tr>
											...
											<td  style="text-align:right;"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="4">
												'.number_format(($total_laboratory),2,'.',',').'
											</font></span></span></td>
										</tr>
										<tr>
				...
				echo number_format((($total_laboratory+$total_prescription+$total_radiology+$total_services)*-1),2,'.',',');
				else echo '0,00';
					echo '
											</font></span></span></td>
										</tr>
...
*/
		...
		$this->sql = "CREATE TEMPORARY TABLE quotation_temporary SELECT
			...
			LEFT JOIN care_tz_drugsandservices ON care_encounter_prescription.article_item_number=care_tz_drugsandservices.item_id
	...
	function GetNewQuotation_Prescriptions($encounter_nr,$in_outpatient, $id_array) {
		...
		else
		{
			$this->sql = "SELECT $anzahl  		care_encounter_prescription.*, " .
	    		...
	    		"							care_tz_drugsandservices.unit_price_3," .
				"							care_tz_drugsandservices.purchasing_class" .
	    		"							from care_encountercare_encounter_prescription
			INNER JOIN care_encounter_prescription ON care_encounter.encounter_nr=care_encounter_prescription.encounter_nr
			INNER JOIN care_test_request_chemlabor_sub ON care_test_request_chemlabor_sub.encounter_nr=care_encounter.encounter_nr
			INNER JOIN care_person On care_person.pid=care_encounter.pid
			INNER JOIN care_tz_drugsandservices ON care_encounter_prescription.article_item_number=care_tz_drugsandservices.item_id
			WHERE care_encounter_prescription.bill_number = 0 $and_in_outpatient
			AND (isnull(care_encounter_prescription.is_disabled)
...
//echo $this->sql;
	...
	function GetNewQuotation_Radiology($encounter_nr, $in_outpatient, &$id_array) {
		...
		if($encounter_nr>0)
		$where_encounter='AND care_test_request_radio.encounter_nr = '.$encounter_nr.' GROUP by care_test_request.batch_nr';
		else
		{
		...
		if(!$_REQUEST['sort'])
		{
			...
			INNER JOIN care_encounter ON care_encounter.encounter_nr=care_test_request_radio.encounter_nr
			INNER JOIN care_person On care_person.pid=care_encounter.pid
			INNER JOIN care_tz_drugsandservices ON care_test_request_radio.test_request=care_tz_drugsandservices.item_id
			...
			$where_encounter
			ORDER BY care_encounter.encounter_date DESC , care_test_request_radio.encounter_nr ASC";
	...
	function GetNewQuotation_Laboratory($encounter_nr, $in_outpatient, &$id_array) {
		...
		$this->sql = "SELECT $anzahl  		care_test_request_chemlabor.*, " .
	    		...
	    		" 			INNER JOIN care_encounter ON care_encounter.encounter_nr=care_test_request_chemlabor.encounter_nr" .
	    		"			INNER JOIN care_person On care_person.pid=care_encounter.pid" .
	    		"			INNER JOIN care_tz_drugsandservices ON care_test_request_chemlabor_sub.paramater_name=care_tz_drugsandservices.partcode" .
	    		"			WHERE care_test_request_chemlabor.bill_number = 0 $and_in_outpatient" .
		...
		else
		{
			...
			INNER JOIN care_encounter ON care_encounter.encounter_nr=care_test_request_chemlabor.encounter_nr
			INNER JOIN care_person On care_person.pid=care_encounter.pid
			INNER JOIN care_tz_drugsandservices ON care_test_request_chemlabor.item_id=care_tz_drugsandservices.item_id
			INNER JOIN care_test_request_chemlabor_sub ON care_test_request_chemlabor.batch_nr=care_test_request_chemlabor_sub.batch_nr
			WHERE care_test_request_chemlabor.bill_number = 0 $and_in_outpatient
			AND (isnull(care_test_request_chemlabor.is_disabled)
			OR care_test_request_chemlabor.is_disabled='')
	...
	function ShowNewQuotationEncounter_Prescriptions($encounter_nr)
	{
		...
		if($result)
		{
		  ...
		  // replace , to . if needed:
				...
				if ($row['purchasing_class']=='labtest' or $row['purchasing_class']=='xray' or $row['purchasing_class']=='service')
				{
		...
		if($result)
		{
		  ...
		  // replace , to . if needed:
				$unit_price_3 = strtr($unit_price_3, ',','.');
					...
					echo  $row['item_description']; echo '</div>
					  			</td width="200">
					  			<td>
					  				<div align="right">
					  				<table border="0" cellpadding="0" width="180">
...
//		echo '<tr><td colspan="8">'.$LDNothingtodo.'</td></tr>';
	...
	function new_reg($encounter,$reg,$prescriber)
	{
		...
		while ($this->res=$this->request->FetchRow()) {
			...
			$this->sql="insert into $this->tbl_prescriptions (encounter_nr,article,article_item_number,price,prescribe_date,prescriber,history,modify_time,dosage)values('".$encounter."','".$item_description."','".$item_id."','".$unit_price."','".date('Y-m-d')."','".$prescriber."','".date('Y-m-d H:i:s')."','".date('Y-m-d H:i:s')."','1')";
