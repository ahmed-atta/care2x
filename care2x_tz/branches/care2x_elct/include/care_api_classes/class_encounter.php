...
* Table name for insurance firms' general data
	...
	*/
	var $tb_chemlabor='care_test_request_chemlabor';
        ...
        /**
	* Table name for radiology data
        * @var string
        */
        var $tb_radio='care_test_request_radio';
	...
	*/
	...
	function SelianPID($enr=0){
                if($this->is_loaded) {
                        return $this->encounter['selian_pid'];
                }else{
                        if($enr) return $this->getValue('selian_pid',$enr,TRUE);
                                else return FALSE;
                }
        }
	...
	*/
	...
	/* Function to search admission info with relation to radiology tests */

	function _searchRadAdmissionBasicInfo($key,$enc_class=0,$add_opt='',$limit=FALSE,$len=30,$so=0){
		global $db,$sql_LIKE;
		$db->debug=FALSE;
		if ($db->debug) echo "_searchRadAdmissionBasicInfo is called with enc_class=".$enc_class."<br>";
		if ($enc_class==2) {
			$this->sql="SELECT DISTINCT p.selian_pid, rc.batch_nr,rc.send_date, e.encounter_nr, e.encounter_class_nr, p.pid, p.name_last, p.name_first, p.date_birth, p.selian_pid, p.sex,p.blood_group
					FROM $this->tb_enc AS e LEFT JOIN $this->tb_person AS p ON e.pid=p.pid " .
	...
	*/
	...
	function searchPatientWithPendingRadResults($key,$len,$so,$sortitem='',$order='ASC'){
		if(!empty($sortitem)){
			$option=" ORDER BY $sortitem $order";
		}
		return $this->_searchRadAdmissionBasicInfo($key,2,$option,TRUE,$len,$so); // 2 = just a list for discharged pat. having pend. Lab Req.
	}
	...
	*/
	function searchInpatientNotInWardBasicInfo($key){
		return $this->_searchAdmissionBasicInfo($key,1,'AND NOT in_ward AND encounter_class_nr=1'); // 1 = outpatient (encounter class)
	}
	/**
...
//		echo $sql;
	...
	* @return boolean
	*/
	function _setCurrentAssignment($enr,$data='',$act='Modified', $outpatient){
		global $dbtype; // $HTTP_SESSION_VARS;
		//echo $data;
	...
	*/
	...
	function resetCurrentDeptForIPD($enr) {
		//echo "<br>resetCurrentDeptForIDP($enr)";
		return $this->_setCurrentAssignment($enr,"current_dept_nr=0,in_dept=0",'ipd admission');
...
//		global $HTTP_SESSION_VARS;
	...
	* @return boolean
	*/
	function ReplaceWard($enr,$ward_nr,$outpatient){
		return $this->_setCurrentAssignment($enr,"current_ward_nr=$ward_nr,in_ward=0",'Replaced ward', $outpatient);
	}
	/**
	...
	*/
	...
	function _discharge_for_admission($enr,$loc_types,$d_type_nr,$date='',$time='', $transferToWard){
		global $dbf_nodate, $dbtype; // $HTTP_SESSION_VARS;
		if(empty($date)) $date=date('Y-m-d');
		if(empty($time)) $time=date('H:i:s');
		$this->sql="UPDATE $this->tb_location
	...
	*/
	...
	function DischargeFromDeptForAdmission($enr,$d_type_nr,$date='',$time='',$rst_enc=1) {
	
		if($this->_discharge_for_admission($enr,"'1','2','3','4','5','6','7','8'",$d_type_nr,$date,$time)) {
				//echo "<br>patient discharged for ipd now<br>";
				return $this->resetCurrentDeptForIPD($enr);
...
//	    global $HTTP_SESSION_VARS;
	...
	* @return mixed adodb record object or boolean
	*/
	function OutPatientsBasic($sort="e.encounter_class_nr",$dept_nr=0){
		global $db;
		$db->debug=0;
		if ( $sort == "selian_pid" )
			$sort = "CAST(selian_pid as UNSIGNED)";

		if($dept_nr) $cond="e.current_dept_nr=$dept_nr AND";
			else $cond='';
			//$cond='';
		$this->sql="SELECT e.encounter_nr,e.encounter_class_nr, e.encounter_date, e.pid,e.insurance_class_nr,p.selian_pid,p.name_last,p.name_first,p.date_birth,p.insurance_ID,p.sex, p.photo_filename,
