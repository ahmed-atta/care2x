...
class dental extends Core {
	...
	*/
	...
	function SaveNewPatientNote($xb2){
		...
		if (($rex[5]=='edit') && ($rex[6]!='')){ # mode && nr !=''
			$db->Execute('DELETE FROM care_encounter_notes WHERE nr='.$rex[6]);
			#print 'Deleted nr='.$rex[6];
			print mysql_error();
		}
	...
	function PrintPatientNotes($pid,$note,$color1,$color2,$recs,$enno){
		...
		$this->sql="SELECT date, notes, create_id, short_notes, encounter_nr, type_nr, nr FROM care_encounter_notes WHERE encounter_nr IN (SELECT encounter_nr FROM care_encounter WHERE pid = '$pid') ORDER BY encounter_nr DESC  limit 0,15";
		$this->result=$db->Execute($this->sql);
