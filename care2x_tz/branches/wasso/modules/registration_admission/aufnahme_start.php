...
if($pid!='' || $encounter_nr!=''){
        ...
        if ($pid)
        {
		  ...
		  if(!$update&&$encounter_nr=$encounter_obj->isPIDCurrentlyAdmitted($pid)){
		     // die('not in here');
		     //$dc = str_replace('','',URL_REDIRECT_APPEND);

		     $dc = substr(URL_REDIRECT_APPEND,0,37);
		     $dc .= '&amp;'. substr(URL_REDIRECT_APPEND,38,strlen(URL_REDIRECT_APPEND));
		     print $dc;
		     ?>
		     <script language="javascript">
		     	window.location.href="<?php print 'aufnahme_daten_zeigen.php'.$dc.'&encounter_nr='.$encounter_nr.'&origin=admit&sem=isadmitted&target=entry';?>";
		     </script>
		     <?php
			  exit;
		  }
...
<script language="JavaScript" type="text/javascript" src="../dental/dental_filling_.js"></script>
...
if(!isset($pid) || !$pid){
	...
	($vct[1]==1)? $dnr = $d_obj->GetPidFromEncounter($encounter_nr) :  $dnr = $encounter_nr;
	($vct[1]==1)? $LDdnr = 'LDRegistryNr'  :  $LDdnr = 'LDCaseNr';


	if ($vct[8]==2){
		$room = 1;
		$apend = ' onchange=" navigate(\'&dpt=\'+this.value,\'../dept_admin/room_getrooms.php\');" style="width:180px; padding:2px;" ';
	}


	$smarty->assign($LDdnr,$LDCaseNr);
	if(isset($encounter_nr)&&$encounter_nr)
		($vct[1]==1)? $smarty->assign('pid',$dnr)  :  $smarty->assign('encounter_nr',$encounter_nr);
...
//							else $sTemp = $sTemp.$result['name'];
			...
			if(!$encounter_nr||$encounter_class_nr==1){
				...
				$sTeemp = '<select';
					($encounter_class_nr==1)? $sTeemp .=  '' : $sTeemp .=  ' disabled=true ';
					$sTeemp .=  ' name="ambulance_nr"> <option value=""></option>';
					...
					if($amb->RecordCount()){
						while($station=$amb->FetchRow()){
							...
							$sTeemp = $sTeemp.'>'.$station[1].'</option>';
						}
					}
					$sTeemp = $sTeemp.'</select>';
				}
			...
			if(!$encounter_nr||$encounter_class_nr==1){
				...
				if($in_ward){
					...
					$sTemp = $sTemp.'<select';
					($encounter_class_nr==1)? $sTemp .=  '' : $sTemp .=  ' disabled=true ';
					$sTemp .=  ' name="current_ward_nr" '.$apend.' >
								<option value=""></option>';
			...
			$sTemp = (($room == 1)&& ($vct[8]==2))?'SELECT DEPARTMENT FIRST <input type="hidden" value="" name="room" id="room">': '<strong>'.$LDNoRooms.'</strong><input type="hidden" value="GENERAL" name="room" id="room">';

			$smarty->assign('sRoomInput',$sTemp);
			...
			if(!$encounter_nr||$encounter_class_nr==2){
				...
				if($in_dept){
					...
					$sTemp = $sTemp.'<select';
							($encounter_class_nr==2)? $sTemp .=  '' : $sTemp .=  ' disabled=true ';
							$sTemp .=  ' name="current_dept_nr" '.$apend.'>
							<option value=""></option>';
			...
			// CONSULTATION FEE
			$cTemp = $cTemp.'<select ';
							($encounter_class_nr==2)? $cTemp .=  '' : $cTemp .=  ' disabled=true ';
							$cTemp .=  ' name="consultation_fee">
							<option value=""></option>';
			...
			// REGISTRATION FEE
			# DONE BY D.MOLLEL from softweb dennis.mollel@yahoo.com
			$rTemp = $rTemp.'<select';
							//($encounter_class_nr==1)? $rTemp .=  '' : $rTemp .=  ' disabled=true ';
							$rTemp .=  '  name="registration_fee">
							<option value=""></option>';
			...
			if ($update&&$current_dept_nr&&$encounter_nr){
				$ns = $cd_obj->GetRoomsAssigned($current_dept_nr);
				$room = $cd_obj->GetPatientRoom($encounter_nr);
				if ($ns->RecordCount()>0){
					$sTemp2= '<select name="room" id="room" style="width:180px; padding:2px;">'.
								'<option value="GENERAL">GENERAL</option>'.
								(($room!='GENERAL')?'<option value="'.$room.'" selected >'.$room.'</option>':'');
						while($n = $ns->FetchRow())
							$sTemp2 .= ($n['name']!=$room)?'<option value="'.$n['name'].'">'.$n['name'].'</option>':'';

					$sTemp2.= '</select>';
				} else
					$sTemp2= '<font color="red"><strong>GENERAL</strong></font> <input type="hidden" value="" name="room" id="room">';

				$sTemp2 = ($vct[8]==2)? $sTemp2.' &nbsp; &nbsp; '.$room : '<strong>'.$LDNoRooms.'</strong><input type="hidden" value="GENERAL" name="room" id="room">';

				$smarty->assign('sRoomInput',$sTemp2);
			}
