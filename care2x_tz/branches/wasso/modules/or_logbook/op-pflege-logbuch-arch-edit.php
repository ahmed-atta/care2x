...
$filetitles=array();
$template=array();
$monat=array("januar","februar","m�rz","april","mai","juni","juli","august","september","oktober","november","dezember");
...
function getlastnum($xyr,$xmon,$xday)
{
 ...
 $xmonat=array("januar","februar","m�rz","april","mai","juni","juli","august","september","oktober","november","dezember");
 while (!sizeof($titles))
  {
	...
	if($xday>1)
	{
		...
		{
			 $xmon--;
			 if(checkdate($xmon,'31',$xyr)) $xday=31; else  if(checkdate($xmon,'30',$xyr)) $xday=30; else if(checkdate($xmon,'29',$xyr)) $xday=29; else $xday=28;
		}
		else $xyr--;
		 ...
		 while (false!==($xfile = readdir($xhandle))) 
			{ 
				$v=strpos($xfile,"_");
    		 	if ($xfile != "." && $xfile != ".." && $v!=0) 
				{ 
       			if(strstr($xfile,".opb")!=false) $titles[]=strtolower($xfile); } 
     		} 
	}
	else break; // if a directory is missing break and return 0
  }
 if (sizeof($titles)>0)
	{
 ...
 case 'save':
	{
		...
		else {$template=get_meta_tags($filename);}
 ...
 default:
	{
		 ...
		 while (false!==($file = readdir($handle))) 
			{ 
				$v=strpos($file,"_");
    		 	if ($file != "." && $file != ".." && $v!=0) 
				{ 
       			if(strstr($file,".opb")!=false) $filetitles[]=strtolower($file);} 
     		} 
		if (sizeof($filetitles)>0)
		{
			...
			$filetitles[0]+=1; 
			if($filetitles[0]<10) $filetitles[0]="0".$filetitles[0];
			$pdata['opnumber']=$filetitles[0];
		}
		else
		{
			$pdata['opnumber']=(getlastnum($pyear,$pmonth,$pday)+1);
			if ($pdata['opnumber']<10) $pdata['opnumber']="0".$pdata['opnumber'];
//			$mode="fresh";
...
function checksubmit()
{
	...
	{
		react=confirm("Die Aufnahmenummer fehlt! M�chten Sie trotzdem speichern?!");	
...
<b>OP Logbuch Eingabefenster</b></FONT>
</TD>
<td align=right bgcolor="navy">

<a href="#"><FONT  COLOR="white"  SIZE=3 face=verdana,arial > Andere Funktionen</a>
&nbsp; 
Hilfe &nbsp; 
<a href="#" onClick="window.parent.opener.focus(); window.parent.close();"><FONT  COLOR="white"  SIZE=3 face=verdana,arial >Schliessen</a>
</td>
</TR>
</TABLE>
...
Name, Vorname<br><INPUT NAME="pname" TYPE="text" VALUE="<?php echo stripcslashes($pdata['pname']); ?>" SIZE="20"><BR>
...
<BR>
	An�sthesist<br><TEXTAREA NAME="anadoc1" Content-Type="text/html"
...
<p>
<a href="#" onClick="window.close()"><img src="../img/close.gif" border=0 width=103 height=24></a>
</ul>
</FORM>
