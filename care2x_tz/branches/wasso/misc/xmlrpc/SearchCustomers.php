<?php
	...
	if (!isset($_POST['submit'])) {
		...
		echo '</table><input type="Submit" name="submit" value="Find Customers"';
		echo '</FORM>';
 	} else {
		foreach ($_POST as $key => $value) {
			if ($value<>'' and $key<>'submit') {
				$field = new xmlrpcval($key);
				$criteria = new xmlrpcval($value);
			}
		}
		$user = new xmlrpcval($weberpuser);
		$password = new xmlrpcval($weberppassword);

		$msg = new xmlrpcmsg("weberp.xmlrpc_SearchCustomers", array($field, $criteria, $user, $password));

		$client = new xmlrpc_client($ServerURL);
		$client->setDebug($DebugLevel);

		$response = $client->send($msg);
		$debtorlist = php_xmlrpc_decode($response->value());
		for ($i=0; $i<sizeof($debtorlist); $i++) {
			echo '<a href="GetCustomer.php?debtorno='.$debtorlist[$i].'">'.$debtorlist[$i].'</a></BR>';
		}
	}
?>
