...
INNER JOIN care_tz_laboratory_param ON care_tz_laboratory_param.nr=care_tz_laboratory_tests.parent WHERE is_enabled=1)";
...
$sql="Select paramater_name as serial_value, parameter_value as add_value ,UNIX_TIMESTAMP(create_time) as date 
from care_test_findings_chemlabor_sub ORDER BY create_time DESC";
...
while (list($u,$v)=each($row)){
	...
	while (list($au, $av) = each($b)) {
		//echo (strpos($av,'check'))."<br>";
		if (strpos($av,'check')===0)
			$sql="update tmp_laboratory_tests set is_positive=1 where TestID=$au AND date='$v[date]'";
			//$sql="INSERT INTO tmp_laboratory_tests (TestID,is_positive, date) VALUES (".$au.",1,".$v['date'].")";
		else
			$sql="update tmp_laboratory_tests set is_positive=0 where TestID=$au AND date='$v[date]'";
			//$sql="INSERT INTO tmp_laboratory_tests (TestID,is_positive, date) VALUES (".$au.",0,".$v['date'].")";
		$db->Execute($sql);
		if ($debug) echo date("F j, Y, g:i a", $v['date'])."<br>";
	}
...
function DisplayLaboratoryTestSummary($group_id, $start_time_frame, $end_time_frame) {
	...
	for ($day=$first_day_of_req_month; $day<=$last_day_of_req_month ; $day++) {
		...
		if ($current_day > time())
			if ($PRINTOUT)
				$table .= "<td><i>".date("j F Y",_get_requested_day($start_time_frame, $day-1))."</i></td>\n";
			else
				$table .= "<td bgcolor=\"#ffffff\"><i>".date("j F Y",_get_requested_day($start_time_frame, $day-1))."</i></td>\n";
		else
