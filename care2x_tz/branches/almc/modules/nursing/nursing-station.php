...
if($ward_ok){
	...
	$smarty->assign('LDAdmissionDate',$LDPatListElements[8]);
	...
	for ($i=$ward_info['room_nr_start'];$i<=$ward_info['room_nr_end'];$i++){
		...
		//for($j=1;$j<=$nr_beds;$j++){
			...
			# Admission Date
			# added by Dennis Mollel from softweb: dennis.mollel@yahoo.com
			$admDate = $ward_obj->_GetAdmissionDate($bed['encounter_nr']);

			if ($admDate) $admDate =  date('d/F/Y H:i', strtotime($admDate));

			$smarty->assign('sAdmissionDate',$admDate);
