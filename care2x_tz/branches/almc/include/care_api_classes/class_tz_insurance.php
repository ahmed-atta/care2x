...
/*
 * Modified by Dennis M Mollel
 * Cybertech and Studios Ltd
 * info@cybertechstudios.com
 *
 * For KCMC-Eye clinic
 *
 * */
  ...
  function ShowOptionInsuranceList($mode,$SHOWALL,$selected) {
	  global $LDID,$LDCompanyName,$LDCompanyName,$LDOptions;
	    $this->insurance_array = $this->GetAllInsurancesAsArray(FALSE, FALSE);
	    while(list($x,$v) = each($this->insurance_array)) {
	      // Do we have to show also the hidden companies?
	      $hd = ($selected==$v['id'])? ' selected ' : '';

			  echo '<option value="'.$v['id'].'" '.$hd.' >'.$v['name'].'</option>';
	  	} // end of while
	    return true;
  }
