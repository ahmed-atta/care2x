...
/* Saves the uploaded file with the given filename into a given path.
*
* @access public
* @param array file in array
* @param string Complete path for the stored file. If empty, the default <var>ATTACH_URL</var> will be used.
* @param string Filename of the stored file. If empty, the original file name will be used.
*/
function AttachFile($file,$fpath,$ac){
	$chmod = '777';
	if ($ac)
		$fpath .= $ac.'/';

	if (!is_dir($fpath)){
	    if (!mkdir($fpath, $chmod, 1)){
	    	return $cr = 1;
	    }
	}

	copy($file['tmp_name'],$fpath.$file['name']);

	return array( $ac.'/'.$file['name'],substr($file['name'],strrpos($file['name'],'.')+1));
...
if(in_array($mode,$modetypes))
{
    ...
    if($dblink_ok) {
		...
		switch($mode)
		{
			...
			{
						if ($_FILES['file'])
							$file = AttachFile($_FILES['file'],ATTACH_URL,$HTTP_COOKIE_VARS[$local_user.$sid]);
					  ...
					  /// the send_stamp is left out to force its auto update
						...
						//	if($folder=="inbox") $folder="sent";
							...
							if($ergebnis=$db->Execute($sql))
							{
								...
								$buf="t=".date('Y-m-d H:i:s')."&r=1&f=$recipient&s=$sub&d=".date('Y-m-d H:i:s')."&at=".$file[0]."&z=".strlen($body_txt)."&u=$uid\r\n";
						...
						if($ergebnis=$db->Execute($sql))
							{
								...
								if($ergebnis->RecordCount())
								{
									...
									while ($mails=$ergebnis->FetchRow())
									{
										...
										$buf="t=".$mails['send_stamp']."&r=0&f=".$mails['sender']."&s=$sub&d=".$mails['send_dt']."&at=".$mails['attachment']."&z=".strlen($mails['body'])."\r\n";
...
{
echo '<ul><form name="mailform" action="'.$thisfile.'" method="post"  ENCTYPE="multipart/form-data" onSubmit="return chkCompose(this)">';
...
<table border=0 cellspacing=1 cellpadding=3 width="80%" height="80%">
    ...
    echo '<tr>
      <td bgcolor="#f3f3f3" align=right><FONT face="Verdana,Helvetica,Arial" size=2 color="#000080">'.$LDattachment.':</td>
      <td  bgcolor="#f3f3f3">';
	  if($sendok) echo '<FONT face="Verdana,Helvetica,Arial" size=2 ><a href="./attachments/'.$file[0].'" target="_blank"><img src="./gui/download.gif" align="absmiddle" border=0> '.$_FILES['file']['name'].'</a>';
	  	else echo '<input type="file" name="file" id="file" />' .
	  			  '<input type="hidden" name = "attacher" value="">';
	echo '
	</td>
    </tr>';
...
if($maxrow)
{
	...
	echo '
       ...
       <td><FONT face="Verdana,Helvetica,Arial" size=2 color="#ffffff">&nbsp;&nbsp;<b>'.$LDattached.'</b>&nbsp;</td>
	        </tr>';
	...
	for($i=0;$i<sizeof($arrlist);$i++)
	   {
		...
		$delbuf="t=$minfo[t]&r=$minfo[r]&f=$minfo[f]&s=$minfo[s]&d=$minfo[d]&at=$minfo[at]&z=$minfo[z]";
     	echo '
       		</td>
			<td>	<input type="checkbox" name="del'.$i.'" value="'.strtr($delbuf," ","+").'"><br>
       		...
       		<td align=center><FONT face="Verdana,Helvetica,Arial" size=1>&nbsp;&nbsp;'.(($minfo['at']!='')?'<img src="./gui/attach.gif" >': '').'</td>
	    	</tr>
