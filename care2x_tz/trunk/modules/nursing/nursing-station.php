		...
		//for($j=1;$j<=$nr_beds;$j++){
			...
			if($edit){
				if(($is_patient)&&!empty($bed['encounter_nr'])){
				    ...
				    $smarty->assign('sDischargeInfoIcon','<a href="javascript:void" onclick="'.((($vct[11]==1)&&($kct==0))?'alert(\'This Patient has no Diagnosis \n You can not Discharge.\')':'release(\''.$bed['encounter_nr'].'\')').'"  title="show info"><img '.createComIcon($root_path,'button_info.gif','0','',TRUE).' alt="show info"></a>');

				    $smarty->assign('sDischargeInfoIcon','<a href="javascript:release_info(\''.$bed['encounter_nr'].'\')" title="show info"><img '.createComIcon($root_path,'button_info.gif','0','',TRUE).' alt="show info"></a>');

					$smarty->assign('sDischargeIcon','<a href="javascript:void(0);" onclick="'.((($vct[11]==1)&&($kct==0))?'alert(\'This Patient has no Diagnosis \n You can not Discharge.\')':'release(\''.$bed['room_nr'].'\',\''.$bed['bed_nr'].'\',\''.$bed['encounter_nr'].'\')').'"><img '.createComIcon($root_path,'bestell.gif','0','',TRUE).' alt="'.$LDReleasePatient.'"></a>');
				}
			}
