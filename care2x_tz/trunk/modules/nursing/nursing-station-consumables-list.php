...
<tbody class="main">
 ...
 <tr valign=top  class="titlebar" >
  <td bgcolor="#99ccff" >
    &nbsp;&nbsp;<font color="#330066"><?php echo 'Consumables for '.$station; ?></font>
...
<?php
			...
			echo '<tr class="titlebar" bgcolor="#99ccff"><td><font color="#330066">Consumables for '.$prescr_date.'</font></td></tr>';
...
//			}
			...
			foreach ($coreObjOuter->result as $rowEncounter){
				...
				$sqlInner = "select * from care_encounter_prescription INNER JOIN care_tz_drugsandservices ON care_tz_drugsandservices.item_id=care_encounter_prescription.article_item_number" .
						" where encounter_nr = $encounterNr $crit and purchasing_class='supplies'";
				...
				foreach ($coreObjInner->result as $rowPrescr){
					...
					$prescr .= $article.'</td><td>'.$dosage.'</td><td>'.$times_per_day.'</td><td>'.$notes.'</td><td><input type="checkbox" name="taken_'.$nr.'"  '.$checked.'>';
					$prescr .= '</td></tr>';
