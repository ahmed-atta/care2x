...
$sql="Select DISTINCT ($tmp_table.description) AS SERVICE , count( $tmp_table.amount ) AS TESTS from $tmp_table " .
		...
		"where ($tmp_table1.purchasing_class LIKE '%surgical_op') " .
		"AND ($tmp_table.date_change>='$start_timeframe' AND $tmp_table.date_change<='$end_timeframe') " .
		"GROUP BY $tmp_table.description ";

//$sql="SELECT DISTINCT(item_description) AS SERVICE , unit_price AS PRICE FROM $tmp_table1 WHERE purchasing_class LIKE '%surgical_op' " ;
...
require_once('gui/gui_reporting_surgeries.php');
?>
