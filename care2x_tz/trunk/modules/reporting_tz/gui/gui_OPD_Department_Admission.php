...
?>
<DIV align="center">
	<h1><?php echo 'OPD Department Admission Summary'; ?><?php echo date('F Y',$start);?></h1>
	<p><?php echo $LDCreationTime; ?><?php echo date("F j, Y, g:i a");?></p>
		...
		// echo "Start time of the script:".date("G:i:s")."<br>";
		 //echo "Looking for test $TestID by time range: day: ".date("d.m.y", $start_timeframe)." starttime: ".date("d.m.y :: G:i:s",$start_timeframe)." endtime: ".date("d.m.y :: G:i:s", $end_timeframe)."<br>";
		  ...
		  while(list($u,$v)=each($res_array) )
		  {
			  ...
			  $sql_patients="SELECT count( * ) AS dept_total FROM $tmp_table WHERE current_dept_nr='$dept_nr' AND date_format( encounter_date, '%d.%m.%y' ) ='$date_person_reg'";
        ...
        <?php
			  ...
			  $sql_patients_total="SELECT count( * ) AS dept_total FROM $tmp_table WHERE current_dept_nr='$dept_nr_total' AND date_format( encounter_date, '%y.%m.%d' ) >='$stdate' AND date_format( encounter_date, '%y.%m.%d' ) <'$enddate'";
