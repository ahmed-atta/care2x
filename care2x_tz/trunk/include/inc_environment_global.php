...
$_COOKIE['ck_login_logged'.$sid]=true;
#
...
if (ini_get('session.use_trans_sid')!=1) {
    define('URL_APPEND', '?lang=en&sid='.session_id().'');
	$not_trans_id=true;
} else {
	# Patch to avoid missing constant
	 define('URL_APPEND', '?lang=en&sid='.session_id().'&ntid=false');
	//define('URL_APPEND','?lang='.$lang);
	$not_trans_id=false;
}
define('URL_REDIRECT_APPEND','?lang=en&sid='.session_id().'');
