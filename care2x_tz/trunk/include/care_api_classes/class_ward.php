...
class Ward extends Encounter {
	...
	*/
	...
	/**
	* used in services list for doctors
	* @var array
	*/
	var$DocServicesList=FALSE;

	/**
	* get patient encounter from the doctors module
	* @var array
	*/
	var $enk=0;
	...
	*/
	...
	function getAll_WardInfo($serv_nr){
		global $db;
		$tsql="SELECT * FROM `care_encounter` WHERE `in_ward`=1 AND `in_dept`=1 AND `medical_service`=".$serv_nr." ORDER BY current_ward_nr DESC";
        return $db->Execute($tsql);
	...
	*/
	function _getWardOccupants($ward_nr,$date){
		...
		if (($this->DocServicesList) &&($this->enk>0))
			$ADsql = ' AND b.encounter_nr='.$this->enk;
		else $ADsql = '';
