...
class Bill extends Encounter {
 ...
 function DisplayArchivedBillHeadlines($no) {
  ...
  function GetBillNrFromEncounter($enc) {
		global $db;
		//$db->debug=TRUE;
		$this->sql="SELECT nr FROM ".$this->tbl_bill." WHERE encounter_nr = $enc";
		if($this->result=$db->Execute($this->sql)){
			if($this->row=$this->result->FetchRow()){
				return $this->row[0];
			} else { return $this->sql;} //false; }
		} else { return $this->sql;} // false; }
  ...
  function GetBillByBatchNr($nr) {
	...
	function DisplayBillHeadline($bill_nr, $batch_nr) {
	  ...
	  echo '<table width="100%"  align="center" border="0" style="background:#F7F7F7;" cellspacing=1 cellpadding=0 >';
  	  echo '' ;
  	  echo	'<tr><td style="border:1px solid #ccc; font:bold 16px Tahoma,Monospace; text-align:center;padding:20px;">'.$bill_head. '</td></tr>  </table>';

  	  echo '<table width="100%"  cellspacing=5 align="center" cellpadding=4>';

  	  echo '<tr>
  				<td style="font:normal 12px Tahoma,Monospace; width:80px;">PID:</td>
  					<td style="font:bold 12px Tahoma,Monospace;">'.$encoded_batch_number.'</td>

  				</tr>';

  				//<tr>
  				//	<td style="font:normal 12px Tahoma,Monospace;">Admission-Nr.:</td>
  				//	<td  style="font:bold 12px Tahoma,Monospace;" bgcolor="#ffffee">'.$enc_number.'</td>
  				//</tr>

  		echo '<tr>
  				<td style="font:normal 12px Tahoma,Monospace;">Surname/Ukoo:</td>
  					<td bgcolor="#ffffee"  style="font:bold 11px Tahoma,Monospace;">'. ucfirst($enc_obj->LastName($enc_number)).'</td>
  				</tr>
  				<tr>
  					<td style="font:normal 12px Tahoma,Monospace;">First name:</td>
  					<td bgcolor="#ffffee"  style="font:bold 12px Tahoma,Monospace;">'. ucfirst($enc_obj->FirstName($enc_number)).'</td>
  				</tr>
  				<tr>
  					<td style="font:normal 12px Tahoma,Monospace;">Date of birth:</td>
  					<td bgcolor="#ffffee"  style="font:bold 12px Tahoma,Monospace;">'.$enc_obj->BirthDate($enc_number).'</td>
  				</tr>
  				<tr>
  					<td style="font:normal 12px Tahoma,Monospace;">'.$LDSex.':</td>
  					<td  style="font:bold 12px Tahoma,Monospace;">'.$enc_obj->Sex($enc_number).'</td>
  				</tr>' .
  						'<tr>
  					<td style="font:normal 12px Tahoma,Monospace;">Bill Date</td>
  					<td  style="font:bold 12px Tahoma,Monospace;">'.date('d-m-Y').'</td>
  				</tr>
  				<tr>
  					<td style="font:normal 12px Tahoma,Monospace;">'.$LDBillNumber.'</td>
  					<td  style="font:bold 12px Tahoma,Monospace;">'.$bill_nr.'</td>
  				</tr>
		<tr>
	</table>';
      return TRUE;
...
//if($sum==0)$sum=$insurance_amt;
	...
	$this->debug = TRUE;
  	echo '<table width="600" border="0" class="table_content">';
  	echo '
  			<tr>
  				<!--<td valign="top" >
				...
				//report submenu
      			...
      			echo '<table border="0" width="100%" style="font:normal 12px Tahoma,Monospace;">';
      			while($bill_elems_row=$billelems->FetchRow()) {
      				$insurance_used += $bill_elems_row['balanced_insurance'];
      				$pos_nr+=1;
      				if($bill_elems_row['is_medicine']==1)
...
function DisplayCompanyPrescriptionBill($bill_nr, $edit_fields){
	...
	$this->debug = TRUE;
		...
		if($sum_lab==0)$sum_lab=$insurance_amt;
				...
				echo '<table border="0" cellspace="2" cellpadding="5">

				  		<tr style="font:bold 11px Tahoma, monospace; text-align:center; background:#EFEFEF; border:1px solid black;">
				  				<td style="border:1px solid #CCCCCC;">Service(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Lab(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">X-Ray(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Dawa(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Proc/Surg(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Dress(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Dental(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Others(TSH)</td>' .
		  						'<td style="border:1px solid #CCCCCC;">Total(TSH)</td>
						</tr>

				  			<tr style="font:normal 11px Tahoma, Monospace;">
				  				<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($service_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($sum_lab),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($xray_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($drug_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($surgery_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($dress_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($dental_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;">'.number_format(($others_amt),2,',','.').'</td>' .
				  				'<td style="border-left:1px solid #CCCCCC;border-bottom:1px solid #CCCCCC;border-right:1px solid #CCCCCC;">'.number_format(($total_amt),2,',','.').'</td>
						</tr>
	  ...
	  if (func_num_args()>3)
		...
		*/
		global $db,$user_origin;
	  	...
	  	echo '<table width="600"  border="0" class="table_content">'; //($printout) ? '<table width="200"  border="1">' :
  		if($specific_bill>0)
  		{
  		...
  		if ($billnumbers) {
    		while($bills=$billnumbers->FetchRow())
    		{
				...
				<tr>
					<td>' .
							'' .
							'<div id="specials" style="width:310px; overflow:hidden; float: left;">' .
							'' .
							'' .
							'' .
							'</div>' .
						...
						echo '<table cellspace="5" align="right" cellspacing="1" cellpadding="4" style="background:#F3F7EE;" >';


						echo '<tr>
								<td style="font:bold 12px Tahoma, Monospace; text-align:right;  padding:5px;">'.$LDSummary.'</td>
								<td>
									<table border="0" >
										<tr>
											<td style="font:normal 12px Tahoma, Monospace; text-align:right;  padding:5px;">
												'.$LDLabTotal.'
											</td>
											<td style=" padding:5px; text-align:right; font:bold 12px Tahoma, Monospace; color:darkblue;">
												'.number_format(($total_laboratory),2,'.',',').'
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td style=" padding:5px; text-align:right; font:bold 12px Tahoma, Monospace; color:black;">
												';
													...
													//poor fund
														...
														$ped = explode('-',$poor_obj->getDentalPoorFund($encounter_nr,$bills['nr']));
										...
										if (($ped[1]=='other') && ($vals[4]==1)){ //poor fund program section START
											   ...
											   echo '' .
												...
												'<tr>
														<td style="font:normal 12px Tahoma, Monospace; text-align:right; padding:5px;">' .
															'Not Paid (Fund):' .
															'</td>
														<td style=" padding:5px; text-align:right; border-bottom: 3px double black; color:maroon; font:bold 12px Tahoma, Monospace;">' .
																'' .
																'' .
																'' .
																'
															'.number_format((($sum_to_pay - $ped[0])),2,'.',',').'
														</td>
													</tr>' .
												'' .
												'' .
										...
										}// poor fund program section ENDS
	        ...
	        if ($printout==FALSE)
		        	...
		        	echo '
		  			    <tr>
		  			  		<td style="text-align:right;">' .
									...
									if ($show_printout_button) echo '<input type="button" class="greenbtn" value="CompanyBill" onclick="javascript:printOut1_'.$bills['nr'].'()">';
		  			  		   echo '<br /><br />' .
		  			  				'' .
		  			  				'<div id="hyd" style="float:right;">' .
		  			  				'' .
		  			  				'' .
		  			  				'' .
		  			  				'</div>';
							...
							if ($vals[4]==1) {
		  			  		   echo '<input' .
		  			  				...
		  			  				'' .
		  			  				'' .
		  			  				' tittle="Click here for Special Billing &raquo; " class="billing" type="button" name="button" value="Special case Billing" onClick="' .
		  			  				'' .
		  			  				'';

		  			  				($sum_to_pay=='0') ? print ' alert(\'You have ZERO BALLANCE. \n\n Action Aborted\n\')' : print 'navigate(\'' .$encounter_nr . '&total='.$sum_to_pay.'&billno='. $bills['nr'] .'&bachno='. $batch_nr .'\',\'../dental/dental_billing_poorfund.php\');';
	         	...
	         	echo '</td>
	  			 ...
	  			 if ($_GET['printout']!= TRUE){
	  			 echo '<tr>' .
	  			  		'<td style="text-align:right;">';

	  			  			   if ($show_printout_button) echo '<a href="javascript:printOut_'.$bills['nr'].'()"><img src="../../gui/img/control/default/en/en_printout.gif" border=0 align="absmiddle" width="99" height="24" alt="Print this form"></a><br /> ';

	  			  			   if ($edit_fields)
	  			  			   	   echo '<a href="billing_tz_pending.php?&mode=done&user_origin='.$user_origin.'&patient='.$_REQUEST['patient'].'&bill_number='.$bills['nr'].'&encounter='.$encounter_nr.'" ';


	  			  			   		if ((intval($ped[0]) != 0) && ($edit_fields)){
			  			  			   	if ($vals[4]==1) { # poor fund is enabled
			  			  			   		if (($vals[5]==1)) { # Letter printing is enabled
			  			  			   			echo 'onClick="window.open(\'../dental/dental_billing_barua.php?encounter='.$encounter_nr.'&total='. $sum_to_pay .'&paid='. $ped[0] .'&bachno='. $batch_nr .'\',\'xx\',\'width=760, height=600,scrollbars=yes\');">';
			  			  			   		}
			  			  			   	}

	  			  			   		}

	  			  			   echo '<img src="../../gui/img/control/default/en/en_done.gif" style="margin:10px 0px 5px 0px;" border=0 align="absmiddle" width="75" height="24" alt="'.$LDDone.'"></a>';

	  		   echo      '<hr />'.$_GET['printout'].'<hr /></td>' .
	  		   		 '</tr>';
	  			 }
    		}
    	}
	  ...
	  if (func_num_args()>3)
		...
		*/
		global $db, $user_origin;
	  	...
	  	echo '
	  	<table width="600" style="border:1px solid #CCCCCC; overflow:visible;">';
  		if($specific_bill>0)
  		{
  			$billnumbers=$this->VerifyBill($specific_bill);
  		}
    		...
    		while($bills=$billnumbers->FetchRow())
    		{
				...
				<tr>
					<td>
						...
						<tr><td style="font:normal 11px Tahoma, Monospace; padding:40px 0px 5px 15px; padding-bottom:20px; ">' .
								'Patient Sign:...........................................' .
							'</td>' .
						'</tr>
				</tr>';
  ...
  function GetNewQuotation_Laboratory($encounter_nr,$in_outpatient) {
    ...
    */
    ...
    $this->sql = "SELECT $anzahl ctr.*, cp.pid, cp.selian_pid, cp.name_first, cp.name_last, cp.date_birth
									...
									AND (isnull(ctr.is_disabled) OR ctr.is_disabled='') $and_in_outpatient
									$where_encounter
    ...
    if($result)
    {
	    while ($row=$result->FetchRow())
	    {
	      ...
	      if(empty($labinfo['anzahl'])) $labinfo['anzahl'] = 0;
				...
				$dept = $obj->GetDepartment($row['encounter_nr']);
          ...
          <tr>
					  ...
					  <td '.$BGCOLOR.' class="td_content"><div align="center">'.$row['pid'].'</div></td>
					  ...
					  <td '.$BGCOLOR.' class="td_content"><div align="center">'.$dept.'</div></td>
					  <td '.$BGCOLOR.' class="td_content"><div align="center">'.

						   '<a href="../../modules/registration_admission/aufnahme_daten_zeigen.php'.URL_APPEND.'&ntid=false&lang=en&target=search&encounter_nr='.$row['encounter_nr'].'" title="Admission data : Click to show data"><img src="../../gui/img/common/default/pdata.gif" alt="Admission data : Click to show data" width="20" border="0" height="20"></a>'
	    ...
	    if(!$counter)
	    	echo '<tr><td colspan="7" align="center">Nothing to do</td></tr>';
	 	}
	  else
	  	echo '<tr><td colspan="7" align="center">Huston we have a problem. Database error :(</td></tr>';
  }
					  				...
					  				<table border="0" cellpadding="0" cellspacing="0" width="250"  class="table_content">
					  					<tr >
					  						...
					  						<td align="right">
					  								...
					  								<br><font size="1">'.$unit_price_description.':'.$unit_price.' '.$LDTSH.'  <input type="radio" name="unit_price_'.$row['nr'].'" value="'.$unit_price.'" checked  onclick="document.forms[0].elements[\'showprice_\' + '.$row['nr'].'].value = '.$unit_price.';calc_article('.$row['nr'].')"></font>
					  								<br><font size="1">'.$unit_price_1_description.':'.$unit_price_1.' '.$LDTSH.'<input type="radio" name="unit_price_'.$row['nr'].'" value="'.$unit_price_1.'" onclick="document.forms[0].elements[\'showprice_\' + '.$row['nr'].'].value = '.$unit_price_1.';calc_article('.$row['nr'].')"></font>
					  								<br><font size="1">'.$unit_price_2_description.':'.$unit_price_2.' '.$LDTSH.'<input type="radio" name="unit_price_'.$row['nr'].'" value="'.$unit_price_2.'" onclick="document.forms[0].elements[\'showprice_\' + '.$row['nr'].'].value = '.$unit_price_2.';calc_article('.$row['nr'].')"></font>
					  								<br><font size="1">'.$unit_price_3_description.':'.$unit_price_3.' '.$LDTSH.'<input type="radio" name="unit_price_'.$row['nr'].'" value="'.$unit_price_3.'" onclick="document.forms[0].elements[\'showprice_\' + '.$row['nr'].'].value = '.$unit_price_3.';calc_article('.$row['nr'].')"> </font><br></td>
					  					</tr>
					  					<tr>
					  						<td>'.$LDDosage.'</td>
