 ...
 * @author Robert Meggle, Dennis Mollel
 * @version beta 1.0.0
...
//------------------------------------------------------------------------------
	...
	function DeleteNewPrescription($nr,$deleted_by,$date,$reason) {
		global $db;
		$debug=FALSE;
		...
		is_disabled = 1, bill_status = 'dropped', disable_id='$deleted_by',disable_date='$date',history = CONCAT(history, '$reason')
		WHERE `nr`=".$nr;
		$db->Execute($this->sql);
		return TRUE;
	}
	...
	function DeleteNewLaboratory($nr,$deleted_by,$date,$reason) {
		global $db;
		$debug=false;
	...
	//------------------------------------------------------------------------------
	function DeleteNewRadiology($nr,$deleted_by,$date,$reason) {
		global $db;
		$debug=FALSE;
...
//		echo '<tr><td colspan="8">'.$LDNothingtodo.'</td></tr>';
	...
	function DisplayAllArchivedBill($txtsearch,$searchtyp, $start_timeframe,$end_timeframe, $trans_id,$max_page_count,$month, $year, $sorttyp) {
		...
		while($bills=$result->FetchRow())
		{
			...
			$total_sum = $this->DisplayBillsInfo($bills,$BGCOLOR);
	...
	<a href="http://www.care2x.org" target=_new>CARE2X ELCT Ver.2.05.3.9</a> :: <a href="../../legal_gnu_gpl.htm" target=_new> License</a> :: <a href=mailto:info@care2x.org>Contact</a>  :: <a href="../../language/en/en_privacy.htm" target="pp"> Our Privacy Policy </a> ::
