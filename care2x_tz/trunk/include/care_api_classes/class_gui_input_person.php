...
*/
			...
			if($mode!='forcesave') {
				...
				/* file not only numeric...
				 * commented by D.Mollel from cybertech
				 */
...
<?php
		...
		if ($vct[2]==2) $LDFileNr = $LDOldNr . ' ' . $LDFileNr;
...
<!-- ADDED FOR KCMC - Eye Department-->
			...
			<td class="reg_item">
				...
				if($errorfilenr)
					echo '<font color="#ff0000">'.$LDEyeNr.'</font><br>
			...
			<td class="reg_input"> <!-- kcmc cybertech -->
			...
			<!-- Diabetic Number -->
			<?php if ($vct[7]==1) { # if has diabetic enabled?>
			<tr>
			<td class="reg_item">
				<FONT SIZE=-1  FACE="Arial"><?php
				if($errorDiabetic)
					echo '<font color="#ff0000">'.$LDDiabeticNr.'</font><br>';
				else
					echo $LDDiabeticNr;
			...
			<td class="reg_input"> <!-- kcmc cybertech -->
				<input type="text" name="diabetic_nr" size=14 maxlength=11 value="<?php echo $diabetic_nr; ?>" onFocus="this.select();">
			</td>
			</tr>
			<?php } # diabetic?>

			<!-- NHIF Number -->
			<?php if ($vct[6]==1) { # if has nhif enabled?>
			<tr>
			<td class="reg_item">
				<FONT SIZE=-1  FACE="Arial"><?php
				if($errorNhif)
					echo '<font color="#ff0000">'.$LDnhifNr.'</font><br>';
				else
					echo $LDnhifNr;
				?>
			</td>
			<td class="reg_input"> <!-- kcmc cybertech -->
				<input type="text" name="nhif_nr" size=14 maxlength=11 value="<?php echo $nhif_nr; ?>" onFocus="this.select();">
			</td>
			</tr>
			...
			<?php if ($vct[3]==1) { # if has ctc Number enabled?>

			<tr>
			<td class="reg_item">
				<FONT SIZE=-1  FACE="Arial"><?php
				if($errorctcnr)
					echo '<font color="#ff0000">'.$LDctcNr.'</font>';
				else
					echo $LDctcNr;

				?>
			</td>
			<td class="reg_input"> <!-- kcmc realsoft -->
				<input type="text" name="ctc_nr" size=14 maxlength=11 value="<?php echo $ctc_nr ?>" onFocus="this.select();">
			</td>
			</tr>
			...
			<td class="reg_item">
				<FONT SIZE=-1  FACE="Arial"><?php
				if($errorhivnr)
					echo '<font color="#ff0000">*'.$LDhivNr.'</font><br>';
				else
					echo $LDhivNr;
				?>
			</td>
			<td class="reg_input"> <!-- kcmc cybertech -->
				<input type="text" name="hiv_nr" size=14 maxlength=11 value="<?php echo $hiv_nr; ?>" onFocus="this.select();">
			</td>
			</tr>
...
<?php
		...
		?>
			...
			<!-- For insuarance companies assignments, by DMM, Cybertech. 4 KCMC-Eye -->
			<tr>
			<td class="reg_item">
			...
			<td colspan=2 class="reg_input"><FONT SIZE=-1  FACE="Arial"><?php if ($errorinsuranceclass) echo '<font color="'.$error_fontcolor.'">'; ?>
				<select name="insuarance_code" id="insuarance_code">
					<option value="0" selected> None </option>
					<?php
						require_once($root_path.'include/care_api_classes/class_tz_insurance.php');
						$geter = new Insurance_tz;
						$geter->ShowOptionInsuranceList(false,false,$insuarance_code);
					?>
				</select>
			</td>
			</tr>
