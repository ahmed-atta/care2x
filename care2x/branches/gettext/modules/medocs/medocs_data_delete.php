						...
						if ($finalcommand=="delete")
								{
									...
									// first renumber the remaining data
										for ($i=$itemname+1;$i<=$linecount;$i++)
										{
											...
											if (!($db->Execute($sql))) echo $sql."  Vorbereitung der Daten zum L�schen ist gescheitert.";
										}
									// then delete the  data
										$sql='DELETE FROM '.$dbtable.' WHERE care_admission_patient_item="'.$itemname.'"';	
										if (!($db->Execute($sql))) echo $sql."  Das L�schen der Daten ist gescheitert.";
...
<?php html_rtl($lang); ?>
<HEAD>
 <TITLE>EDV - Zugangsberechtigunge l�schen</TITLE>
</HEAD>
...
<tr>
<td bgcolor="navy" >
<FONT  COLOR="white"  SIZE=+2  FACE="Arial"><STRONG>&nbsp;Patientendaten l�schen</STRONG></FONT>
...
Wollen Sie die folgende Patientendaten wirklich l�schen?<p>
...
<img src="../img/delete.gif"> <INPUT type="submit" name="versand" value="Ja, l�schen"></font></FORM>
