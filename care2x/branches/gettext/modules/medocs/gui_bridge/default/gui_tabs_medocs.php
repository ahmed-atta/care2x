...
if(!isset($notabs)||!$notabs){
	...
	$smarty->assign('pbNew','<a href="medocs_start.php'.URL_APPEND.'&target=entry"><img '.createLDImgSrc($root_path,$img,'0').' title="'.$LDAdmit.'" class="fadeOut" ></a>');

	if($target=="search") $img='such-b.gif';
		else{ $img='such-gray.gif'; }

	$smarty->assign('pbSearch','<a href="medocs_data_search.php'.URL_APPEND.'&target=search"><img '.createLDImgSrc($root_path,$img,'0').' title="'.$LDSearch.'"  class="fadeOut" ></a>');
