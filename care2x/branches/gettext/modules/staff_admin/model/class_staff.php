...
class Staff extends Core {
	...
	*/
	var $dpoc_fields=array('nr',
									...
									'create_time');
	/**
	* Field names of care_staff_assignment
	* @var array
	*/
	var $assign_fields=array('nr',
									...
									'create_time');
	/**
	* Field names of care_staff
	* @var array
	*/
	var $staff_fields=array('nr',
									'short_id',
									'pid',
									'job_type_nr',
									'job_function_title',
	...
	*/
	...
	/**
	* Sets the core object to point to the care_staff_assignment table and field names.
	* @access public
	*/
	...
	/**
	* Sets the core object to point to the care_staff table and field names.
	* @access public
	*/
	...
	*/
	function getDoctorsOfDept($dept_nr=0){
		if(!$dept_nr) return FALSE;
			else return $this->_getAllstaff(1,17,$dept_nr); // 1= dept (location), 17 = doctor (role)
	}
	/**
	* Returns information of all nurses of a department.
	*
	...
	* @param int Department number
	*/
	function _getAllstaff($loc_type_nr,$role_nr=0,$dept_nr){
	    global $db, $dbf_nodate;
		$row=array();
		
		$sql="SELECT a.nr, a.staff_nr, ps.job_function_title, p.name_last, p.name_first, p.date_birth, p.sex
	...
	* @param int Department number
	*/
	function _getstaffById($staff_nr){
	    global $db, $dbf_nodate;
		$row=array();
	...
	* @param int Role number
	*/
	function _getAllstaffByRole($role_nr=0){
	    global $db, $dbf_nodate;
		$row=array();
	...
	function getstaffInfo($nr){
		...
		$sql="SELECT ps.*,p.*,
				...
				FROM $this->tb AS ps, 
						...
						$this->tb_cphone AS c ON c.staff_nr=$nr
				WHERE ps.nr='$nr'
				 AND ps.pid=p.pid";
	...
	* @return mixed adodb record object  or boolean
	*/
	function searchstaffBasicInfo($key,$oitem='name_last',$odir='ASC',$limit=FALSE,$len=30,$so=0){
		global $db, $sql_LIKE;
	...
	/**
	* Search similar to searchstaffBasicInfo but returns a limited number of rows.
	*
	* For detailed structure of returned data, see <var>searchstaffBasicInfo()</var> method.
	* @access public
	...
	* @return mixed adodb record object  or boolean
	*/
	function searchLimitstaffBasicInfo($key,$len,$so,$oitem,$odir){
		return $this->searchstaffBasicInfo($key,$oitem,$odir,TRUE,$len,$so);
	}
	/**
	...
	*/
	function LastName(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['name_last'];
	}
	/**
	...
	*/
	function FirstName(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['name_first'];
	}
	/**
	...
	*/
	function BirthDate(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['date_birth'];
	}
	/**
	...
	*/
	function PhoneKey(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['phone_pk'];
	}
	/**
	...
	*/
	function InPhone1(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['inphone1'];
	}
	/**
	...
	*/
	function InPhone2(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['inphone2'];
	}
	/**
	...
	*/
	function InPhone3(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['inphone3'];
	}
	/**
	...
	*/
	function ExPhone1(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['exphone1'];
	}
	/**
	...
	*/
	function ExPhone2(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['exphone2'];
	}
	/**
	...
	*/
	function ExPhone3(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['exphone3'];
	}
	/**
	* Returns first dept
	*/
	function Dept1(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['bereich1'];
	}
	/**
	* Returns second dept
	*/
	function Dept2(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['bereich2'];
	}
	/**
	...
	*/
	function formattedAddress_DE(){
	    //if(!$this->is_loaded) return FALSE;
		return $this->staff_data['addr_str'].' '.$this->staff_data['addr_str_nr'].'<br>'.$this->staff_data['addr_str_zip'].' '.$this->staff_data['citytown_name'];
	}
	/**#@-*/
	/**
