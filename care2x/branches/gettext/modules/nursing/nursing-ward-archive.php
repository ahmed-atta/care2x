...
function update()
{
	...
	{
	jbuf=parseInt(jbuf);
	var urltarget="nursing-ward-archive.php<?php echo URL_REDIRECT_APPEND; ?>&pmonth="+mbuf+"&pyear="+jbuf;
...
{	echo '<tr>';
		...
		for($n=0;$n<6;$n++)
		{
			...
			if($pyear.$mn.$dn<=date(Ymd))  echo '
			  <a href="nursing-quickview.php'.URL_APPEND.'&from=arch&dept='.$dept.'&edit=0&pday='.$dn.'&pmonth='.$mn.'&pyear='.$pyear.'">';
			echo $daynumber[$j];
	...
	echo '><b>&nbsp;';
	if($pyear.$mn.$dn<=date(Ymd))  echo '<a href="nursing-quickview.php'.URL_APPEND.'&from=arch&dept='.$dept.'&edit=0&pday='.$dn.'&pmonth='.$mn.'&pyear='.$pyear.'">';
	echo $daynumber[$j];
	if($pyear.$mn.$dn<=date(Ymd))  echo '</a>';
	echo '</b></td>'; 	$j++;
	echo '</tr>';
