...
{
	urlholder="nursing-ward-bed-occupancy.php?sid=<?php echo $sid; ?>&s=<?php echo $station; ?>&rm="+room+"&bd="+bed+"<?php echo "&py=".$pyear."&pm=".$pmonth."&pd=".$pday."&tb=".str_replace("#","",$cfg['top_bgcolor'])."&tt=".str_replace("#","",$cfg['top_txtcolor'])."&bb=".str_replace("#","",$cfg['body_bgcolor'])."&d=".$cfg['dhtml']; ?>";
...
<a href="#" onClick=window.close()><img src="../img/fenszu.gif" border=0  <?php if($cfg['dhtml'])echo'class="fadeOut" >';?></a></td></tr>
<tr valign=top >
...
if($deffile)
		 	{
			 echo'<font face="verdana,arial" size="2"  color=red>Die Belegung f�r heute ist noch nicht erstellt!</font><br>';
			 }
...
for ($i=$statdata['startnumber'];$i<=$statdata['endnumber'];$i++)
 {
   for($j='a';$j<='b';$j++)
	...
	if($buf[0]!="!") echo 'getinfo(\''.$buf[0].'\',\''.$buf2.'\') title="Click f�r mehr Info">';
	else echo 'unlock(\''.strtoupper($j).'\',\''.$i.'\') title="Click f�r Info bzw. zum Aufheben der Sperre.">'; //$j=bed   $i=room number
