<?php
/**
* This script is included by the barcode_label_single_large.php script for Turkish Dr Bulent Potur 2005
*/
...
#       
	...
	if($location['bed_nr']) $locstr.=' '.strtoupper(chr($location['bed_nr']));
	$locstr.=' '.tr2uni($admit_type).' '.tr2uni($LDInsShortID[$result['insurance_class_nr']]);
	...
	if($ttf_ok){
    	...
    	ImageTTFText($label,11,0,$lmargin,$tmargin+108,$eblack,$arial,ucfirst(tr2uni($result['addr_zip'])).' '.tr2uni($result['citytown_name']));
		# Sex
		...
		# Insurance co name
    	ImageTTFText($label,14,0,$lmargin,$tmargin+150,$eblack,$arial,tr2uni($ins_obj->getFirmName($result['insurance_firm_id'])));
		# Location
