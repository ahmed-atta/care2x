...
*/  
	...
	if($ttf_ok){
    	ImageTTFText($im,12,0,$lmargin,$tmargin+10,$black,$arial,tr2uni($LDPrintPortraitFormat));
    	ImageTTFText($im,12,0,$lmargin,$tmargin+25,$black,$arial,tr2uni($LDClickImgToPrint));
	}else{
	...
	$dataline1=tr2uni($result['name_first']).' '.tr2uni($result['name_last']);
	$dataline2=tr2uni($result['date_birth']);
	$dataline3=tr2uni($result['current_ward']).' '.tr2uni($result['current_dept']).' '.tr2uni($result['insurance_co_id']).' '.tr2uni($result['insurance_2_co_id']);
	//$ttf_ok=0;
	
	
	if($ttf_ok){
    	ImageTTFText($namelabel,11,0,$lmargin,$tmargin,$nm_black,$arial,tr2uni($dataline1));
    	ImageTTFText($namelabel,10,0,$lmargin,$tmargin+13,$nm_black,$arial,tr2uni($dataline2));
    	ImageTTFText($namelabel,10,0,$lmargin,$tmargin+26,$nm_black,$arial,tr2uni($dataline3));
	}else{
