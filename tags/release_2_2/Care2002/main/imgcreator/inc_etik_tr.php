<?php
/**
* This script is included by the barcode-etik.php for Turkish
*Created by Dr. Bülent Potur, bpotur@gmail.com 2005-11-01
*/
	include_once($root_path.'include/inc_ttf_tr2uni.php');
		...
		$locstr.=' '.$admit_type.' ';
		$locstr.=tr2uni($LDInsShortID[$result['insurance_class_nr']]);
    ...
    ImageTTFText($im,9,0,195,11,$black,$arial,tr2uni("$LDAdmitTime:"));
    ImageTTFText($im,10,0,205,24,$black,$arial,formatDate2Local($result['encounter_date'],$date_format,1,1));
    ImageTTFText($im,9,0,290,11,$black,$arial,tr2uni("$LDDept").':');
    ImageTTFText($im,10,0,295,24,$black,$arial,tr2uni($result['current_dept']));
	ImageTTFText($im,9,0,385,11,$black,$arial,tr2uni("$LDRoomNr").':');
    ImageTTFText($im,10,0,390,24,$black,$arial,$result['current_room']);
	ImageTTFText($im,9,0,480,11,$black,$arial,tr2uni("$LDAdmitType").':');
    ImageTTFText($im,10,0,485,24,$black,$arial,tr2uni($result['encounter_type']));
    ImageTTFText($im,9,0,290,36,$black,$arial,tr2uni("$LDBday:"));
    ...
    ImageTTFText($im,9,0,480,36,$black,$arial,tr2uni("$LDCivilStat").':');
    ImageTTFText($im,10,0,485,49,$black,$arial,tr2uni($result['civil_status']));
	ImageTTFText($im,9,0,290,61,$black,$arial,tr2uni("$LDPhone").':');
    ImageTTFText($im,10,0,350,75,$black,$arial,$result['phone_1_nr']);
	ImageTTFText($im,9,0,290,88,$black,$arial,tr2uni("$LDInsurance:"));
    ImageTTFText($im,10,0,300,110,$black,$arial,tr2uni($result['insurance_co_id']));
	ImageTTFText($im,9,0,290,205,$black,$arial,tr2uni("$LDInsuranceNr:"));
    ImageTTFText($im,10,0,360,205,$black,$arial,$result['insurance_nr']);

	// name & address
   	ImageTTFText($im,9,0,5,50,$black,$arial,tr2uni("$LDNameAddr:"));
   	ImageTTFText($im,12,0,10,80,$black,$arial,tr2uni($result['name_first']).' '.tr2uni($result['name_last']));
   	ImageTTFText($im,12,0,10,100,$black,$arial,ucfirst(tr2uni($result['addr_str'])).' '.tr2uni($result['addr_str_nr']));
   	ImageTTFText($im,12,0,10,115,$black,$arial,ucfirst(tr2uni($result['addr_zip'])).' '.tr2uni($result['name']));

	// diagnosis, therapie, 
    ImageTTFText($im,12,0,10,235,$black,$arial,tr2uni("$LDDiagnosis").'                    : '.tr2uni($result['referrer_diagnosis']));
    ImageTTFText($im,12,0,10,250,$black,$arial,tr2uni("$LDRecBy").':   '.tr2uni($result['referrer']));
    ImageTTFText($im,12,0,10,265,$black,$arial,tr2uni("$LDTherapy").'     :'.tr2uni($result['referrer_recom_therapy']));
    ImageTTFText($im,12,0,10,280,$black,$arial,tr2uni("$LDSpecials").':'.tr2uni($result['referrer_notes']));
    ImageTTFText($im,12,0,10,295,$black,$arial,tr2uni("$LDAdmitDiagnosis").'                  :'.tr2uni($result['referrer_diagnosis']));
    ImageTTFText($im,12,0,10,310,$black,$arial,tr2uni("$LDInfo2").' :'.tr2uni($result['info2']));
	    // -- print date, religion, 
   ImageTTFText($im,9,0,5,345,$black,$arial,tr2uni("$LDPrintDate").': ');
    ImageTTFText($im,10,0,5,358,$black,$arial,formatDate2Local(date('Y-m-d'),$date_format));
    ImageTTFText($im,9,119,345,$black,$arial,tr2uni("$LDReligion").': ');
    ImageTTFText($im,10,0,119,358,$black,$arial,tr2uni($result['religion']));
    ImageTTFText($im,9,0,238,345,$black,$arial,tr2uni("$LDTherapyType").': ');
    ImageTTFText($im,10,0,238,358,$black,$arial,tr2uni($result['therapy_type']));
    ImageTTFText($im,9,0,352,345,$black,$arial,tr2uni("$LDTherapyOpt").': ');
    ImageTTFText($im,10,0,352,358,$black,$arial,tr2uni($result['therapy_option']));
	ImageTTFText($im,9,0,466,345,$black,$arial,tr2uni("$LDServiceType").': ');
    ImageTTFText($im,10,0,466,358,$black,$arial,tr2uni($result['service_type']));
	 # Bill payer information
	 #  Note: the address format is german
     #
     ImageTTFText($im,9,0,5,155,$black,$arial,tr2uni("$LDBillInfo").': ');	
	}else{
    ...
    ImageString($im,1,290,26,tr2uni("$LDBday").':',$black);
    ...
    ImageString($im,1,290,78,tr2uni("$LDInsurance").':',$black);
    ...
    ImageString($im,3,10,70,tr2uni($result['name_first']).', '.tr2uni($result['name_last']),$black);	
    ...
    ImageCopy($im,$bc,110,28,9,9,170,37);
	 ...
	 {
	 	if($ttf_render){
     	 	ImageTTFText($im,12,0,10,170,$black,$arial,tr2uni($result['name_first']).' '.tr2uni($result['name_last']));
   	     	ImageTTFText($im,12,0,10,185,$black,$arial,ucfirst(tr2uni($result['addr_str'])).' '.tr2uni($result['addr_str_nr']));
    	 	ImageTTFText($im,12,0,10,200,$black,$arial,ucfirst(tr2uni($result['addr_zip'])).' '.tr2uni($result['name']));
	 }else{
         	ImageString($im,3,10,160,tr2uni($result['name_first']).' '.tr2uni($result['name_last']),$black);
         	ImageString($im,3,10,175,tr2uni($result['addr_str']).' '.tr2uni($result['addr_str_nr']),$black);
         	ImageString($im,3,10,190,tr2uni($result['addr_zip']).' '.tr2uni($result['name']),$black);
		}
	}
	else
	{
	...
	if($ttf_ok){
    	...
    	ImageTTFText($label,16,0,$lmargin,$tmargin+56,$eblack,$arial,tr2uni($result['name_first']).' '.tr2uni($result['name_last']));
		# Date of birth
    	...
    	ImageTTFText($label,11,0,$lmargin,$tmargin+93,$eblack,$arial,ucfirst(tr2uni($result['addr_str']).' '.tr2uni($result['addr_str_nr'])));
		# Address, zip, city/town name
    	ImageTTFText($label,11,0,$lmargin,$tmargin+108,$eblack,$arial,ucfirst(tr2uni($result['addr_zip'])).' '.tr2uni($result['name']));
		# Sex
    	ImageTTFText($label,14,0,$lmargin,$tmargin+130,$eblack,$arial,strtoupper($result['sex']));
		# Family name, repeat print
    	ImageTTFText($label,14,0,$lmargin+30,$tmargin+130,$eblack,$arial,tr2uni($result['name_last']));
		# Insurance co name
    	ImageTTFText($label,14,0,$lmargin,$tmargin+150,$eblack,$arial,$ins_obj->getFirmName(tr2uni($result['insurance_firm_id'])));
		# Location
    	...
    	ImageString($label,5,10,40,tr2uni($result['name_first']).' '.tr2uni($result['name_last']),$black);
    	ImageString($label,3,10,55,formatDate2Local($result['date_birth'],$date_format),$black);
     	ImageString($label,4,10,75,tr2uni($result['addr_str']).' '.tr2uni($result['addr_str_nr']),$black);
     	ImageString($label,4,10,90,$result['addr_zip'].' '.$result['name'],$black);
	
    	ImageString($label,5,10,125,strtoupper($result['sex']),$black);
    	ImageString($label,5,30,125,tr2uni($result['name_last']),$black);
	...
	if($ttf_ok){
   	ImageTTFText($label2,9,0,10,45,$black,$arial,$full_en);
    	...
    	ImageTTFText($label2,11,0,10,60,$black,$arial,tr2uni($result['name_first']).'');
    	ImageTTFText($label2,11,0,10,75,$black,$arial,tr2uni($result['name_last']));
    	...
    	ImageTTFText($label2,10,0,10,109,$black,$arial,$ins_obj->getFirmName(tr2uni($result['insurance_firm_id'])));
    	...
    	ImageString($label2,4,10,50,tr2uni($result['name_first']).',',$black);
    	ImageString($label2,4,10,65,tr2uni($result['name_last']),$black);
    	...
    	ImageString($label2,3,10,100,tr2uni($result['insurance_co_id']),$black);
	...
	if($ttf_ok){
    	...
    	ImageTTFText($label3,12,0,10,34,$black,$arial,tr2uni($result['name_first']).' ');
    	ImageTTFText($label3,12,0,10,49,$black,$arial,tr2uni($result['name_last']));
    	ImageTTFText($label3,10,0,10,64,$black,$arial,formatDate2Local($result['date_birth'],$date_format));
    	ImageTTFText($label3,10,0,10,84,$black,$arial,tr2uni($addr1));
    	ImageTTFText($label3,10,0,10,99,$black,$arial,tr2uni($addr2));
	}else{
