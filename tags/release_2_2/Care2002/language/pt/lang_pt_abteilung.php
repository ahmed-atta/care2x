<?php
$LDPageTitle='Departmentos';
$LDDeptTxt='Departmento';
$LDOpenHrsTxt='Horário de expediente';
$LDChkHrsTxt='Horário de Checkup Médico';
$LDClk4Info='Clique aqui para mais informação sobre';
$LDDayTxt='Dia';

/**
* A small dictionary:
* ENT = Eye, Nose, Throath
* OB-Gyne = Obstetrics-Gynecology
*/
$dept=array('Cirurgia Geral',
					'Cirurgia de Emergência',
					'Cirurgia Plástica',
					'Otorrinolaringologia',
					'Oftalmologia',
					'Patologia',
					'Ginecologia-Obstetrica',
					'Fisioterapia',
					'Medicina Interna',
					'Oncologia',
					'Manutenção',
					'Unidade de Cuidados Intermédios',
					'Unidade de Cuidados Intensivos',
					'Laboratório Médico',
					'Ambulatório de Emergência',
					'Ambulatório Geral',
					'Sonografia',
					'Diagnóstico Nuclear',
					'Ambulatório de Medicina Interna');
					
/**
* Do not translate the $target variable
*/
$target=array('dept_generalsurgery',
					'dept_emergency',
					'dept_plasticsurgery',
					'dept_ent',
					'dept_eyesurgery',
					'dept_pathology',
					'dept_gynecology',
					'physiotherapy',
					'dept_internalmed',
					'dept_oncology',
					'dept_techservice',
					'dept_IMCU',
					'dept_ICU',
					'dept_lab',
					'unfamb',
					'allamb',
					'sono',
					'nuklear',
					'inmed');

$LDBackTxt='Anterior';

$LDOpenDays=array('Segunda',
					'Terça',
					'Quarta',
					'Quinta',
					'Sexta',
					'Sábado',
					'Domingo');
$LDOpenTimes=array('8.30 - 21.00', //Mo
					'8.30 - 21.00', //Di
					'8.30 - 21.00', //Mi
					'8.30 - 21.00', //Do
					'8.30 - 21.00', //Fr
					'8.30 - 21.00', //Sa
					'8.30 - 21.00'); //So
$LDVisitTimes=array('12.30 - 15.00 , 19.00 - 21.00', //Mo
					'12.30 - 15.00 , 19.00 - 21.00', //Di
					'12.30 - 15.00 , 19.00 - 21.00', //Mi
					'12.30 - 15.00 , 19.00 - 21.00', //Do
					'12.30 - 15.00 , 19.00 - 21.00', //Fr
					'12.30 - 15.00 , 19.00 - 21.00', //Sa
					'12.30 - 15.00 , 19.00 - 21.00'); //So
?>
