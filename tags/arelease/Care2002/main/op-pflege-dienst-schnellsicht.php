...
for ($i=0;$i<sizeof($abtarr);$i++)
	...
	{
		$sql="SELECT list FROM nursing_dept_personell_quicklist WHERE
				dept='".$abtarr[$i]."'";
		//print $sql;
		if($ergebnis=mysql_query($sql,$link))
       		{
	...
	<img src="../img/mans-gr.gif" width=12 height=15>&nbsp;<a href="javascript:popinfo(\''.$aelems[l].'\',\''.$aelems[f].'\',\''.$aelems[b].'\',\''.$abtarr[$i].'\')" title="'.$LDClk2Show.' '.$LDExtraInfo.'"><b>';
	...
	<img src="../img/mans-red.gif" width=12 height=15>&nbsp;<a href="javascript:popinfo(\''.$relems[l].'\',\''.$relems[f].'\',\''.$relems[b].'\',\''.$abtarr[$i].'\')" title="'.$LDClk2Show.' '.$LDExtraInfo.'"><b>';
	//if ($relems[l]!="") print $relems[l].', '.$relems[f];
	print $relems[s].'</b></a></td>';
	print '<td><font face="verdana,arial" size="2" >';
	if ($relems[l]!="") 
	{
