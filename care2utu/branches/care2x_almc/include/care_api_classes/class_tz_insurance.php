    ...
    */
    ...
    if (!$JUST_CONTRACTED)
      $this->sql="SELECT com.id, com.name,com.contact, com.email, com.phone_code, com.phone_nr, com.po_box, com.city, com.hide_company_flag,
        ins.cancel_flag,ins.start_date,ins.end_date,ins.id as i_id 

	FROM 

	( SELECT max(care_tz_insurance.id) as i_id, care_tz_company.id ,name,contact, email, phone_code, phone_nr, po_box, city, hide_company_flag,
        cancel_flag,care_tz_insurance.start_date,care_tz_insurance.end_date FROM

	care_tz_company
	 INNER JOIN care_tz_insurance  ON care_tz_insurance.company_id=care_tz_company.id  GROUP BY care_tz_company.id

	) as ins_q	

	INNER JOIN care_tz_company as com ON
	ins_q.id      =  com.id
	INNER JOIN care_tz_insurance as ins ON
	ins_q.i_id      =  ins.id

      ".$hide_sql1."   ORDER BY com.id ASC ";
      ...
      $this->sql="SELECT
                    ...
                    care_tz_insurance.start_date,
                    care_tz_insurance.end_date,
                    care_tz_company.invoice_flag,
		    care_tz_insurance.cancel_flag,
...
//------------------------------------------------------------------------------
  ...
  global $LDID,$LDCompanyName,$LDCompanyName,$LDStartDate,$LDEndDate,$LDStatus,$LDOptions;
    /**
  ...
  function ShowContractsOfCompany($company_id) {
    ...
    */
    $this->contract_array = $this->GetContractsForCompanyAsArray($company_id);
		...
		urlholder="insurance_company_tz_contracts_works.php?id=" + id;
                testprintout=window.open(urlholder,"printout","width=630,height=600,menubar=no,resizable=yes,scrollbars=yes");
	...
	<td align="center" width="40">'.$LDOptions.'</td>
    </tr>
    ';
    if(is_array($this->contract_array))
	    while(list($x,$v) = each($this->contract_array))
	    {
	      ...
	      <td><div align="center">'.$LDYes.'<input type="radio" '.$cancel_flag_yes.' name="cancel_'.$v['id'].'"  id="cancel_'.$v['id'].'"  value="yes">  <input type="radio" '.$cancel_flag_no.' name="cancel_'.$v['id'].'"  id="cancel_'.$v['id'].'" value="no"> '.$LDNo.'</td>
	...
	<td><div align="center"><a href="javascript:Update('.$v['id'].')">
<img src="../../gui/img/common/default/play_one.gif" alt="Edit Contract" border="0"></a></td>
...
function CheckCurrentContractValidity($company_id) {
   ...
   */
    ...
    $this->sql = "SELECT distinct(company_id) as company_id,  name FROM care_tz_insurance
										...
										AND invoice_flag=1 ORDER by name ASC";
