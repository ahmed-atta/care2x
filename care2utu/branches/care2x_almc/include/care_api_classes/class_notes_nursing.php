...
class NursingNotes extends Notes {
	...
	*/			
	...
	/* Display all the previos entered notes

        */
	
	function getDailyNotesTypes($enr,$pid){
                if($this->_getNotes("encounter_nr=$enr","ORDER BY date,time")){
                        return $this->result;
                }else{
                        return false;
                }
	...
	*/			
	...
	/**
        ...
        * @return boolean
        */
        function saveDailyNotesType(&$data){
                $buf;
                if(empty($data)){
                        return false;
                }else{
                        ...
                        $this->data_array['location_nr']=$data['dept_nr'];
                        $this->data_array['location_id']=$data['station'];
                        $this->data_array['type_nr']=$data['types'];
                        $this->data_array['date']=date('Y-m-d');
                        $this->data_array['time']=date('H:i:s');
                        return $this->_insertNotesFromInternalArray($this->data_array['type_nr']);
                }
        }
