...
class Prescription extends Core {
	...
	*/
	...
	function getDrugList($class, $is_enabled ) {
		...
		$debug = FALSE;
		($debug)?$db->debug=TRUE:$db->debug=FALSE;
		$this->sql='SELECT  item_description as description, item_id as drug_id, partcode as stockid FROM '.
			...
			if ($is_transmit_to_weberp_enable==1) {
				...
				$items=$weberp_obj->get_stock_items_from_category_property($types[$typeno], 'X');
