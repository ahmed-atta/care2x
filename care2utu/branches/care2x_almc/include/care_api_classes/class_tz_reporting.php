  ...
  */
  function class_selianreport() {
    ...
    function DisplayBillingTableHead(){
    	global $PRINTOUT;
    	global $LDDailyFinancialRecordOPD,$LDDate,$LDAmb,$LDBed,$LDConsult,$LDConsum,$LDDental,$LDDress,$LDDrugs,$LDFile,$LDICU,$LDLab,$LDMort,$LDProcSurg,
$LDImaging,$LDWChair,$LDTotal;
		...
		$table_head = "<tr>\n";
		if (!$PRINTOUT)
			$table_head .= "<td bgcolor=\"#ffffaa\" colspan=\"16\" align=\"center\">".$LDDailyFinancialRecordOPD."</td>\n";
		else
			$table_head .= "<td colspan=\"16\" align=\"center\">".$LDDailyFinancialRecordOPD."</td>\n";
		$table_head.="</tr>\n";
		...
		if (!$PRINTOUT) {
			...
			$table_head .= "<td bgcolor=\"#CC9933\">".$LDDress."</td>\n" ;
			...
			$table_head .= "<td bgcolor=\"#CC9933\">".$LDWChair."</td>\n" ;
			...
			$table_head .= "<td>".$LDDress."Dress</td>\n" ;
			...
			$table_head .= "<td>".$LDWChair."W/Chair</td>\n" ;
...
$LDImaging,$LDWChair,$LDTotal;
		...
		$table_head = "<tr>\n";
		if (!$PRINTOUT)
			$table_head .= "<td bgcolor=\"#ffffaa\" colspan=\"16\" align=\"center\">Cash Financial Summary</td>\n";
		else
			$table_head .= "<td colspan=\"16\" align=\"center\">Cash  Financial Summary</td>\n";
		$table_head.="</tr>\n";
...
function DisplayInsuranceBillingTableHead(){
    	global $PRINTOUT;
    	global $LDDailyFinancialRecordOPD,$LDDate,$LDAmb,$LDBed,$LDConsult,$LDConsum,
    	       $LDDental,$LDDress,$LDDrugs,$LDFile,$LDICU,$LDLab,$LDMort, $LDProcSurg,$LDImaging,$LDWChair,$LDTotal;
		...
		$table_head = "<tr>\n";
		if (!$PRINTOUT)
			$table_head .= "<td bgcolor=\"#ffffaa\" colspan=\"16\" align=\"center\">Insurance Financial Summary</td>\n";
		else
			$table_head .= "<td colspan=\"16\" align=\"center\">Insurance  Financial Summary</td>\n";
		$table_head.="</tr>\n";
...
function DisplayInsuranceCompanyBillingTableHead($insurance_id){
	global $db;
        global $PRINTOUT;
        global $LDDailyFinancialRecordOPD,$LDDate,$LDPatient,$LDAmb,$LDBed,$LDConsult,$LDConsum,$LDDental,$LDDress, $LDDrugs,$LDFile,$LDICU,
               $LDLab,$LDMort,$LDProcSurg,$LDImaging,$LDWChair,$LDTotal;
		...
		$insurancename = $sql_insurancename['name'];
                ...
                $table_head = "<tr>\n";
                if (!$PRINTOUT)
                        $table_head .= "<td bgcolor=\"#ffffaa\" colspan=\"17\" align=\"center\"> Financial Summary for $insurancename </td>\n";
                else
                        $table_head .= "<td colspan=\"17\" align=\"center\">Financial Summary for $insurancename </td>\n";
                $table_head.="</tr>\n";
			...
			$table_head .= "<td>".$LDFile."File</td>\n" ;
                        ...
                        $table_head .= "<td>".$LDProcSurg."Proc/Surg</td>\n" ;
                        $table_head .= "<td>".$LDImaging."Imaging</td>\n" ;
			$table_head .= "<td>".$LDWChair."WChair</td>\n" ;
                        $table_head .= "<td>".$LDTotal."Total</td>\n" ;
                        $table_head.="</tr>\n";
                }
                echo $table_head;
...
/**
    ...
    function DisplayARVBillingTableHead(){
    	global $PRINTOUT;
    	global $LDDailyFinancialRecordOPD,$LDDate,$LDPatientName,$LDAmb,$LDBed,$LDConsult,$LDConsum,$LDDental,$LDDress,$LDDrugs,
    	       $LDFile,$LDICU,$LDLab,$LDMort,$LDProcSurg,$LDImaging,$LDWChair,$LDTotal;
		...
		$table_head = "<tr>\n";
		if (!$PRINTOUT)
			$table_head .= "<td bgcolor=\"#ffffaa\" colspan=\"17\" align=\"center\">DailyFinancialRecord ART Fund</td>\n";
		else
			$table_head .= "<td colspan=\"17\" align=\"center\">DailyFinancialRecord ART Fund</td>\n";
		$table_head.="</tr>\n";
	...
	function _get_amount_of($start_timeframe,$day,$filter,$total_summary) {
		...
		global $db;
		global $LDAmb,$LDBed,$LDConsult,$LDConsum,$LDDental,$LDDress,$LDDrugs,$LDFile,$LDICU,$LDLab,$LDMort,$LDProcSurg,$LDRadio,$LDWChair,$LDTotal;
		...
		switch ($filter) {
			case "amb": //
				...
				$sql_filter="WHERE purchasing_class='service' AND item_number like 'A%' AND item_number<>'R02' AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
				
			case "bed": //Consultation
				if ($debug) echo "bed<br>";

				$sql_filter="WHERE purchasing_class='service' AND item_number like 'B%' AND item_number<>'R02' AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
			...
			case "proc/surg":
				...
				$sql_filter="WHERE ( purchasing_class='eye-service' OR purchasing_class='minor_proc_op' OR purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR purchasing_class='surgical_op') AND $curr_day_start <=date_change AND $curr_day_end>=date_change ";
				break;
			...
			// returns: all patients, what got the service item R02
				if ($debug) echo "wchair<br>";
   				$sql_filter="WHERE purchasing_class='service' AND item_number like 'W%' AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
	...
	function _get_cash_amount_of($start_timeframe,$day,$filter,$total_summary) {
		...
		switch ($filter) {
			...
			case "dress":
				if ($debug) echo "dress<br>";
				$sql_filter="WHERE purchasing_class='minor_proc_op' 
				AND insurance_id=0 AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
			...
			case "proc/surg":
				if ($debug) echo "proc/surg<br>";
				$sql_filter="WHERE ( purchasing_class='eye-service' OR purchasing_class='minor_proc_op' OR purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR purchasing_class='surgical_op') 
				AND insurance_id=0 AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
			...
			// returns: all patients, what got the service item R02
				if ($debug) echo "wchair<br>";
   				...
   				$sql_filter="WHERE purchasing_class='service' AND item_number like 'W%' 
				AND insurance_id=0 AND $curr_day_start <=date_change AND $curr_day_end>=date_change";
				break;
...
function _get_insurance_amount_of($start_timeframe,$day,$filter,$total_summary) {
		...
		global $db;
		global $LDAmb,$LDBed,$LDConsult,$LDConsum,$LDDental,$LDress,$LDDrugs,$LDFile,$LDICU,$LDLab,$LDMort,$LDProcSurg,$LDRadio,$LDWChair,$LDTotal;
...
function _get_arv_amount_of($start_timeframe,$day,$encounternr,$filter,$total_summary) {
	...
	if ($total_summary) {
		...
		switch ($filter) {
			...
			case "dress": //dressing
                        if ($debug) echo $LDDress."<br>";
                                $sql_filter="WHERE purchasing_class='minor_proc_op' AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y')  and encounter_nr='$encounternr'";
                                break;
			...
			case "file": //file
                        ...
                        if ($debug) echo $LDFile."<br>";
                                $sql_filter="WHERE purchasing_class='service' AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y')  and encounter_nr='$encounternr'";
                                break;
			...
			case "proc/surg": //
                        if ($debug) echo $LDProcSurg."<br>";
                                $sql_filter="WHERE (purchasing_class='eye-service' OR purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR         purchasing_class='surgical_op')
AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y')  and encounter_nr='$encounternr'";
			...
			case "wchair"://returns
				if ($debug) echo "wchair<br>";
   				$sql_filter="WHERE purchasing_class='service' AND item_number like 'W%' AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y')  and encounter_nr='$encounternr'";
				break;
...
function _get_prepaid_amount_of($start_timeframe,$day,$insuranceid,$filter,$total_summary) {
		...
		$sql_filter="WHERE (  purchasing_class='drug_list') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  insurance_id='$insuranceid'";
			...
			case "proc/surg":
				if ($debug) echo "proc/surg<br>";
				$sql_filter="WHERE ( purchasing_class='eye-service' OR purchasing_class='minor_small_op' OR purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR purchasing_class='surgical_op') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  insurance_id='$insuranceid'";
				break;
			...
			case "wchair":
				if ($debug) echo "wchair<br>";
				$sql_filter="WHERE purchasing_class='service' AND item_nuber like 'W%'  AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  insurance_id='$insuranceid'";
				break;
...
function _get_insurance_company_amount_of($start_timeframe,$day,$encounternr,$insuranceid,$filter,$total_summary) {
		...
		case "amb": 
			...
			case "dress":
				if ($debug) echo "dress<br>";
				$sql_filter="WHERE purchasing_class='minor_proc_op' AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y')  and (item_number='P31' OR item_number='P32')and description NOT LIKE '%ultra%sound%' AND
		...
		encounter_nr='$encounternr' AND  insurance_id='$insuranceid'";
			...
			case "proc/surg":
				if ($debug) echo "proc/surg<br>";
				$sql_filter="WHERE ( purchasing_class='eye-service' OR purchasing_class='minor_proc_op' OR purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR purchasing_class='surgical_op') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  encounter_nr='$encounternr' AND  insurance_id='$insuranceid'";
			...
			case "wchair":
				if ($debug) echo "wchair<br>";
				$sql_filter="WHERE purchasing_class='service' AND item_number like 'W%'  AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND 
				 encounter_nr='$encounternr' AND  insurance_id='$insuranceid'";
			...
			case "total":
				if ($debug) echo $LDTotal."<br>";
				$sql_filter="WHERE  from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  encounter_nr='$encounternr' AND  
...
function _get_dental_prepaid_amount_of($start_timeframe,$day,$insuranceid,$filter,$total_summary) {
		...
		$sql_filter="WHERE ( purchasing_class='drug_list') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  insurance_id='$insuranceid'";
			...
			case "proc/surg":
				...
				$sql_filter="WHERE ( purchasing_class='eye-service' OR purchasing_class='minor_proc_op' OR 
purchasing_class='obgyne_op' OR purchasing_class='ortho_op' OR purchasing_class='surgical_op') AND from_unixtime($curr_day_start,'%d-%m-%Y') <= from_unixtime(date_change,'%d-%m-%Y') AND from_unixtime($curr_day_start,'%d-%m-%Y') >= from_unixtime(date_change,'%d-%m-%Y') AND  insurance_id='$insuranceid' and description NOT LIKE '%ultra%sound%'";
				break;
...
function _Create_art_financial_tmp_master_table($start_timeframe,$end_timeframe) {
	...
	function DisplayBillingTestSummary($start_timeframe, $end_timeframe){
		...
		for ($day=$first_day_of_req_month; $day<=$last_day_of_req_month ; $day++) {
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_amount_of($start_timeframe,$day,"wchair", FALSE).$italic_tag_close."</td>\n";
		...
		$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_amount_of($start_timeframe,1,"wchair", TRUE).$italic_tag_close."</td>\n";
...
function DisplayCashBillingTestSummary($start_timeframe, $end_timeframe, $admission){
		...
		for ($day=$first_day_of_req_month; $day<=$last_day_of_req_month ; $day++) {
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_cash_amount_of($start_timeframe,$day,"wchair", FALSE).$italic_tag_close."</td>\n";
...
function DisplayInsuranceBillingTestSummary($start_timeframe, $end_timeframe, $admission){
		...
		for ($day=$first_day_of_req_month; $day<=$last_day_of_req_month ; $day++) {
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_amount_of($start_timeframe,$day,"wchair", FALSE).$italic_tag_close."</td>\n";
		...
		while($db_row_encounter_nr=$db_ptr_encounter_nr->FetchRow())
		{
			...
			$pid=$db_row_encounter_nr['pid'].'<br>';
			 ...
			 $hospitalnr=$db_row_hospitalnr['name_last'].'  '.$db_row_hospitalnr['name_first'];
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,$day,$encounternr,$insuranceid,"dress", FALSE).$italic_tag_close."</td>\n";
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,$day,$encounternr,$insuranceid,"wchair", FALSE).$italic_tag_close."</td>\n";
...
/*		if (!$PRINTOUT)$bg_color="#CC9933";
				...
				$table .= "<td bgcolor=\"$bg_color\" align = \"center\"><strong>&sum;</strong></td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$hospitalnr.$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"amb", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"bed", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"consult", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"consum", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"dental", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"dress", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"drugs", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"file", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"icu", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"lab", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"mort", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"procsurg", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"radio", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"wchair", TRUE).$italic_tag_close."</td>\n";
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_insurance_company_amount_of($start_timeframe,1,"total", TRUE).$italic_tag_close."</td>\n";
...
function DisplayCompaniesBillingTestSummary($start_timeframe, $end_timeframe){
                global $db;
                global $PRINTOUT;
                global $LDLookingforFinancialReports,$LDstarttime,$LDendtime;
                $first_day_of_req_month=0;
        ...
        function DisplayCompaniesBillingResultRows($start_timeframe, $end_timeframe){
			 ...
			 while($db_row_encounter_nr=$db_ptr_encounter_nr->FetchRow())
			{
			 ...
			 $hospitalnr=$db_row_hospitalnr['name_last'].'  '.$db_row_hospitalnr['name_first'];
				...
				$table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_arv_amount_of($start_timeframe,$day,$encounternr,"icu", FALSE).$italic_tag_close."</td>\n";
                                ...
                                $table .= "<td bgcolor=\"$bg_color\" align=\"right\">".$italic_tag_open.$this->_get_arv_amount_of($start_timeframe,$day,$encounternr,"wchair", FALSE).$italic_tag_close."</td>\n";
