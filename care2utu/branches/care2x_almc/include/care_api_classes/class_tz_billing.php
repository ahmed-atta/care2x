...
//		echo $this->sql;
	...
	function StorePrescriptionItemToBill($pid, $prescriptions_nr, $bill_number, $insurance, $pricelist_no){
		...
		INSERT INTO $this->tbl_bill_elements " .
				...
				"		prescriptions_nr)" .
				"
		VALUES (".$bill_number.", '".time()."', 0, 1, 0, 0, 0, '".$row['total_dosage']."', '".$row['total_dosage']."', ".$row['times_per_day'].",". $row['days'].", ".$price.", '".$row['article']."', '".$row['notes']."', ".$row['article_item_number'].", '".$insurance."', '".$contract['id']."', '".$row['nr']."')";
		...
		while ($row=$result->FetchRow()) {
           ...
           $this->sql ="
			...
			"	(" .
				...
				"		prescriptions_nr)" .
				"
			VALUES (".$bill_number.", '".time()."', 1, 0, 0, 0, 1, 1, 1, 0, 0, ".$price.", '".$row['item_description']."', 0, ".$row['p_item_id'].", '".$insurance."', '".$contract['id']."', '".$row['prescrip_nr']."')";
	...
	function StoreRadiologyItemToBill($pid, $batch_nr, $bill_number, $insurance, $pricelist_no){
		...
		INNER JOIN care_tz_drugsandservices
		ON care_tz_drugsandservices.item_description = care_test_request_radio.test_request
 ...
 '".$row['item_description']."', ".$row['item_id'].", '".$insurance."', '".$contract['id']."', ".$row['batch_nr'].")";
		...
		if ($billnumbers) {
			while($bills=$billnumbers->FetchRow())
			{
				...
				echo '
	  	  			<tr>
	  	  				<td valign="top"><span class=SpellE><span style="font-family:&quot;20 cpi&quot;"><font size="-1">';
				$total_other = $this->DisplayOtherBill($bills['nr'],$edit_fields);
				echo '
		  	      		</font></span></span></td>
		  	      	</tr>';
				}
				...
				<tr>
					<td>';
				echo ($printout) ? '<table  width="100%" border="1" align="left">' : '<table width="100%" border="0" align="right" 
class="headline">';


				echo '<tr>
								<td ><span style="font-family:&quot;20 cpi&quot;"><font size="4">'.$LDSummary.'</font></span></span></td></tr>
								<td>
				...
				echo number_format((($total_laboratory+$total_prescription+$total_radiology+$total_services+$total_other)*-1),2,'.',',');
				else echo '0,00';
					echo '
											</font></span></span></td>
										</tr>
...
date_birth, encounter_date , encounter_class_nr
