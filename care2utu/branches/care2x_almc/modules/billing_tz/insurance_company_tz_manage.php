			...
			if (isset($_GET['save']))
			{
				...
				{ //update existing insurance
					...
					if (!is_numeric($max_pay) )
					{
						...
						$coreObj->sql="UPDATE care_tz_company SET name=\"$name\", contact=\"$contact_person\", email=\"$email\",phone_code=\"\", phone_nr=\"$phone\", po_box=\"$po_box\", city=\"$city\" WHERE id=$insurance_ID";
						echo $coreObj->sql;
						$db->execute($coreObj->sql);
					}
				}
...
<?php require ("gui/gui_insurance_company_tz_manage.php");?>
