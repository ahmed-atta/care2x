...
($_SERVER["PHP_SELF"].URL_APPEND.'&sid='.$sid.'&pn='.$pn.'&edit=1').'\'" value="'.$LDNotesReports.'">'.
		...
		echo ''.
			'<input  style="width:220px; overflow:hidden; float:left; margin:0px 0px 7px 6px;" 
	type="button" onClick="window.location.href=\''.$root_path.'modules/registration_admission/show_notes.php'.URL_REDIRECT_APPEND.'$sid='.$SID.'&pn='.$pn.'&lang=en&ntid=false&externalcall=true&help_site=patient_charts&target=search&1=1&backpath='.urlencode($_SERVER["PHP_SELF"].URL_APPEND.'&pn='.$pn).'\'" value="'.$LDReports.'">'.
