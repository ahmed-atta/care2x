...
if($rows){
	...
	while ($patient=$opat_obj->FetchRow()){
			...
			if($patient['selian_pid']){
				...
				if(isset($sg)&&$sg) $smarty->assign('sPatNr',eregi_replace($sg,"<font color=#ff0000><b>".ucfirst($sg)."</b></font>",$patient['selian_pid']));
					else $smarty->assign('sPatNr',$patient['selian_pid']);
			}
			...
			if ($patient['encounter_nr']) $smarty->assign('sAdmissionDate',$encounter_date);
			...
			$insurance_name = $ins_obj->GetName_insurance_from_id($patient['insurance_ID']);
	        	$smarty->assign('sInsuranceType',$insurance_name);
