...
class Ward extends Encounter {
	...
	*/
	...
	# added by Dennis Mollel from softweb dennis.mollel@yahooo.com
	# get admited date from encounter

	function _GetAdmissionDate($encounter_nr) {
		global $db;
		//$db->debug=TRUE;
		$this->sql="SELECT encounter_date FROM care_encounter where encounter_nr=".$encounter_nr;
		if($this->result=$db->Execute($this->sql)){
			if($this->row=$this->result->FetchRow()){
				return ($this->row['encounter_date']);
			} else { return false; }
		} else { return false; }
	}
