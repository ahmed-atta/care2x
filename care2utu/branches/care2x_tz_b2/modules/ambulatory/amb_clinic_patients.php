...
if ($vct[8]==2){
	$smarty->assign('sRoomTop',str_replace('~room~',strtoupper($room), $LDroomSeparatorTop));
	$smarty->assign('sRoomBottom',$LDroomSeparatorBottom);
}
...
function Transfer(pn,pw){
	if(confirm("<?php echo $LDSureTransferPatient."-" ?>")){
		urlholder="amb_clinic_transfer_select.php<?php echo URL_REDIRECT_APPEND ?>&pn="+pn+"&room="+pw+"&pat_station=&dept_nr=<?php echo $dept_nr ?>&station=<?php echo $station ?>";
...
if($rows){
	...
	while ($patient=$opat_obj->FetchRow()){
			...
			if (($vct[8]==2) && ($room!=$patient['room'])) continue;
			...
			if($edit){
				...
				*/
				...
				$smarty->assign('sTransferIcon','<a href="javascript:Transfer(\''.$patient['encounter_nr'].'\',\''.$room.'\')"><img '.createComIcon($root_path,'xchange.gif','0','',TRUE).' alt="'.$LDTransferPatient.'"></a>');
	...
	if ($COUNTER==0)
		$smarty->assign('sNoPatientRow',$LDRoomNoPatient);
