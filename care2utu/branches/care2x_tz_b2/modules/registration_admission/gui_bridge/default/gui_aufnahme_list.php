...
?>
			<input name="encounter_class_nr" type="radio" onclick="
					<?php if ($result['class_nr']==2) { ?>$('#wards').attr('disabled', 'disabled');<?php } ?>
					<?php if ($result['class_nr']==1) { ?>$('#wards').removeAttr('disabled'); <?php } ?>
					"	value="<?php echo $result['class_nr']; ?>"  <?php if($encounter_class_nr==$result['class_nr']) echo 'checked'; ?>>
<?php
			$LD=$result['LD_var'];
...
?>
	...
	<tr bgcolor="white">
		...
		<td colspan=2 bgcolor="#eeeeee">
			<select name="current_ward_nr" id="wards">
...
<?php
    // Discharged options added by D.Mollel from Softweb tech
?>

	<tr>
		<td background="<?php echo createBgSkin($root_path,'tableHeaderbg3.gif'); ?>">&nbsp;<?php echo $LDDischargedDate; ?>:
		</td>
		<td bgcolor="#eeeeee">
			<input name="disch_date_start" type="text" size=10 maxlength=10  value="<?php if(!empty($disch_date_start)) echo @formatDate2Local($disch_date_start,$date_format);  ?>"  onBlur="IsValidDate(this,'<?php echo $date_format ?>')" onKeyUp="setDate(this,'<?php echo $date_format ?>','<?php echo $lang ?>')">
			<a href="javascript:show_calendar('aufnahmeform.disch_date_start','<?php echo $date_format ?>')">
			<img <?php echo createComIcon($root_path,'show-calendar.gif','0','absmiddle'); ?>></a><font size=1>[<?php
			$dfbuffer="LD_".strtr($date_format,".-/","phs");
			echo $$dfbuffer;
		?>] </font>
		...
		<td bgcolor="#eeeeee">
			<nobr>&nbsp;<?php echo $LDTo ?>:
			<input name="disch_date_end" type="text" size=10 maxlength=10 value="<?php if(!empty($disch_date_end)) echo @formatDate2Local($disch_date_end,$date_format);  ?>"  onBlur="IsValidDate(this,'<?php echo $date_format ?>')" onKeyUp="setDate(this,'<?php echo $date_format ?>','<?php echo $lang ?>')">
			<a href="javascript:show_calendar('aufnahmeform.disch_date_end','<?php echo $date_format ?>')">
			<img <?php echo createComIcon($root_path,'show-calendar.gif','0','absmiddle'); ?>></a><font size=1>[<?php
			$dfbuffer="LD_".strtr($date_format,".-/","phs");
			echo $$dfbuffer;
		?>] </font>
	...
	<tr bgcolor="white">
		<td background="<?php echo createBgSkin($root_path,'tableHeaderbg3.gif'); ?>">&nbsp;<?php echo $LDDischargedType; ?>:
	</td>
		<td colspan=2 bgcolor="#eeeeee">
			<select name="dischaged_type" id="notward">
				<option value=""><?php echo $dischaged_type; ?></option>
<?php
		$discharge_types=&$enc_obj->getDischargeTypesData();
		while($dis_type=$discharge_types->FetchRow()){
				echo '<option value="'.$dis_type['nr'].'" ';
				if(isset($dischaged_by)&&($dischaged_by==$dis_type['nr'])) echo 'selected';
				echo '>'.$dis_type['name'].'</option>';
...
<?php
	createTR( 'dischaged_by',  $LDDischargedBy,$dischaged_by);
	?>
