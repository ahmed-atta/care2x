...
//							else $sTemp = $sTemp.$result['name'];
			...
			$sTemp = ($room == 1)?'SELECT DEPARTMENT FIRST <input type="hidden" value="" name="room" id="room">': '<B>'.$LDNoRooms.'</B>';
			...
			if ($update&&$current_dept_nr&&$encounter_nr){
				...
				if ($ns->RecordCount()>0){
					...
					$sTemp2= '<font color="red"><b>GENERAL</b></font> <input type="hidden" value="" name="room" id="room">';

				$smarty->assign('sRoomInput',$sTemp2.' &nbsp; &nbsp; '.$room);
			}
