...
if($task=="insert") {
	...
	{//echo print_r($_POST);
		...
		if ($debug) echo "looking for: ".$x."  ".$v."<br>";
		...
		if(strstr($x,"modepres_"))
		{
			...
			if($_POST['modepres_'.$prescriptions_nr]=='bill')
			{
				...
				if ($debug) echo "showprice_$prescriptions_nr :".$_POST['showprice_'.$prescriptions_nr]."<br>";
				$price=$_POST['showprice_'.$prescriptions_nr];
				//echo $_POST['price_'.$prescriptions_nr];
				if ($debug) echo 'times per day: '.$_POST['times_per_day_'.$prescriptions_nr].'<br>';
				if ($debug) echo 'days: '.$_POST['days_'.$prescriptions_nr].'<br>';
				if ($debug) echo 'unit_price: '.$_GET['unit_price'].'<br>'; // <=== This is the selected pricelist!!!
		...
		elseif(strstr($x,"modelab_"))
		{
			...
			if ($dosageOld != $dosageNew)
				...
				$bill_obj->StoreLaboratoryItemToBill($pid, $labtest_nr, $batch_nr, $new_bill_number, $_POST['insurance_'.$labtest_nr]);
		...
		{
			if ($debug) echo "looking for rad ...<br>";
			...
			if ($dosageOld != $dosageNew)
				...
				if ($debug) echo "Radiology: allocate2insurance(".$new_bill_number.", $radtest_nr,".$_POST['insurance'].");";
				if ($debug) echo "radtest nr.".$radtest_nr."<br>";
				if ($debug) echo "billnumber: $new_bill_number<br>";
				//if ($debug) echo "radtest nr.: ".$_POST['insurance_'.$radtest_nr]."<br>";
				if ($debug) echo "insurance: ".$_POST['insurance']."<br>";

				$bill_obj->StoreRadiologyItemToBill($pid,$radtest_nr,$new_bill_number, $_POST['insurance_'.$radtest_nr]);

				$bill_obj->UpdateBillNumberNewRadiology($radtest_nr,$new_bill_number);

				if ($_POST['insurance']!=-1)
					$insurance_obj->allocateRadiologyItemsToinsurance($new_bill_number, $radtest_nr,$_POST['insurance']);
			}
	...
	if($billcounter){
		...
		$row=$result->FetchRow();
		$batch_nr=$row['pid'];
		header("location: billing_tz_edit.php".URL_APPEND."&batch_nr=".$batch_nr."&bill_number=".$new_bill_number."&user_origin=quotation");
	}
	else
	{
		if($deletecounter>0)
...
if ($debug) echo "<br>nothing to do - just show what we have...<br>";
