...
<!--		{foreach from=$invoiceItems_cat item=invoiceItem_cat }  -->
	...
	<!--		    <td>{$invoiceItem_cat.category}</td><td>{$preference.pref_currency_sign}{$invoiceItem_cat.gross_total}</td><td>{$preference.pref_currency_sign}{$invoiceItem_cat.tax_amount}</td><td>{$invoiceItem_cat.total}</td>	 -->
	        </tr>
		  <tr  class='itemised' >       
	                    <td></td>
				<td colspan=5>
