...
if(isset($mode) && !empty($mode)) {
	...
	if($mode == 'savenew') {
		...
		if(empty($HTTP_POST_VARS['status'])) $HTTP_POST_VARS['status']=' ';
		$HTTP_POST_VARS['modify_id']=$HTTP_SESSION_VARS['sess_user_name'];
		$HTTP_POST_VARS['group_id'] = '-1';
		$HTTP_POST_VARS['sort_nr'] = rand(1, 99);
		$HTTP_POST_VARS['id'] = str_replace(" ","_",strtolower($HTTP_POST_VARS['name']));
...
{
	urlholder="labor_test_group_edit?sid=<?php echo "$sid&lang=$lang" ?>&nr="+nr;
	editgroup_<?php echo $sid ?>=window.open(urlholder,"editgroup_<?php echo $sid ?>","width=500,height=600,menubar=no,resizable=yes,scrollbars=yes");
}
// -->
</script>
...
<form action="<?php echo $thisfile; ?>" method="post" name="groupedit">
...
/*	echo '<tr><td  class="a12_b" bgcolor="#fefefe">&nbsp;'.$LDID.'</td>
	...
	echo '<tr><td  class="a12_b" bgcolor="#fefefe">&nbsp;'.$LDShowParam.'</td>
			...
			<select name="status">
			  ...
			  <option value="hidden"'; 
				 if ($tg['status']=='hidden') echo "selected";
				 echo '>'.$LDHide.'</option>
			  <option value="deleted"';
				 if ($tg['status']=='deleted') echo "selected";
