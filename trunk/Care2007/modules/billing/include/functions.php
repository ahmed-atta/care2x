...
function calc_customer_total($customer_id) {
	...
	global $tb_prefix;
		...
		$sql ="SELECT IF ( ISNULL( SUM({$tb_prefix}invoice_items.total)) ,  '0', SUM({$tb_prefix}invoice_items.total)) AS total 
		FROM
...
function show_custom_field($custom_field,$custom_field_value,$permission,$css_class_tr,$css_class1,$css_class2,$td_col_span,$seperator,$invoice_id) {
	global $tb_prefix;
	/*
...
EOD;
						...
						if($custom_field_number == 1) {
							$display_block .= "<select name='customField$custom_field_number'>";
							
							$personells = getPersonell();

							foreach($personells as $personell) {
								//print_r($department);
								$display_block .= " <option ";
								if($invoice_id != NULL) {
									if($invoice['custom_field1']==$personell[nr]) {
										$display_block .= " selected ";
									}
								}
								
								$display_block .= "value='$personell[nr]'>$personell[nr] $personell[name_last] $personell[name_first]</option>";
							}
							$display_block .= "</select>";
						}
						else if($custom_field_number == 2) {
							$display_block .= "<select name='customField$custom_field_number'>";
							...
							foreach($departments as $department) {
								...
								if(isset($invoice_id)) {
									if($invoice['custom_field2']==$department['name_formal']) {
										$display_block .= " selected ";
									}
								}
								
								$display_block .= "value='$department[name_formal]'>$department[name_formal]</option>";
							}
							$display_block .= "</select>";
						}
						else {
							$display_block .= "<input type=text name='customField$custom_field_number' value='$custom_field_value' size=25></input>";
						}
