...
function medianValue($paramValue,$pName){
	...
	switch ($diff) {
		case ( ($diff >= 1) and ($diff <= 30 ) ) :
			if($pName->fields['lo_bound_n']!=null&&$pName->fields['hi_bound_n']!=null) $txt.=htmlspecialchars($pName->fields['hi_bound_n'])."<p><br>".htmlspecialchars($pName->fields['lo_bound_n']);
			break;
		case ( ($diff >= 31) and ($diff <= 360 ) ) :
			if($pName->fields['lo_bound__y']&&$pName->fields['hi_bound_y']) $txt.=htmlspecialchars($pName->fields['hi_bound_y'])."<p><br>".htmlspecialchars($pName->fields['lo_bound_y']);
			break;
		case ( $diff >= 361) and ($diff <= 5040 ) :
			if($pName->fields['lo_bound_c']&&$pName->fields['hi_bound_c']) $txt.=htmlspecialchars($pName->fields['hi_bound_c'])."<p><br>".htmlspecialchars($pName->fields['lo_bound_c']);
			break;
		case $diff > 5040 :
			if($patient['sex']=='m')
				if($pName->fields['lo_bound']&&$pName->fields['hi_bound']) $txt.=htmlspecialchars($pName->fields['hi_bound'])."<p><br>".htmlspecialchars($pName->fields['lo_bound']);
			elseif($patient['sex']=='f')
				if($pName->fields['lo_bound_f']&&$pName->fields['hi_bound_f']) $txt.=htmlspecialchars($pName->fields['hi_bound_f'])."<p><br>".htmlspecialchars($pName->fields['lo_bound_f']);	
			break;
	}
	return $txt;
}
