...
class Lab extends Encounter {
    ...
    */
    ...
    function UpdateParams($paramarray){
        ...
        if ($debug) echo "class_lab::UpdateParams($paramarray)";
    ...
    /**
    * Sets the core table name and field names to the
care_test_param table.
    * @access public
    */
...
encounter_nr='$enc_nr' AND status<>'hidden' ORDER BY batch_nr ASC";
    ...
    function GetUserDataByEncounter($enc_nr){
        ...
        $this->sql="SELECT * FROM care_person cp, care_encounter ce
WHERE
        ce.pid = cp.pid AND
    ...
    /**
    * Gets the batch number of a given encounter number and  job
id.
    * @access public
    * @param int Encounter number
    ...
    */
    ...
    function BatchNr($enc_nr,$job_id,$grp_id){
        global $db;
        $this->sql="SELECT batch_nr,send_date FROM
$this->tb_find_chemlab WHERE encounter_nr='$enc_nr' AND
...
H:i:s')." ".$HTTP_SESSION_VARS['sess_user_name']."\n")."
...
($this->dead_stat)";
    ...
    * The array contains the test result data with index keys as
outlined in the <var>$fld_find_chemlab</var> array.
    * @access public
...
batch_nr=$bn";
    ...
    /**
    * Gets the test result data basing on encounter number, job id,
and test group id keys.
    *
    * The returned adodb record object contains row of array.
    * The array contains the test result data with index keys as
outlined in the <var>$fld_find_chemlab</var> array.
    ...
    */
    function getResult($job_id,$grp_id,$enc_nr=''){
        ...
        if($grp_id=='all')
            $this->sql="SELECT * FROM $this->tb_find_chemlab WHERE
encounter_nr='$this->enc_nr' AND job_id='$job_id' AND
status<>'hidden'";
        else
            $this->sql="SELECT * FROM $this->tb_find_chemlab WHERE
encounter_nr='$this->enc_nr' AND job_id='$job_id' AND
group_id='$grp_id' AND status<>'hidden'";
        if($this->result=$db->Execute($this->sql)){
            if($this->rec_count=$this->result->RecordCount()) {
                return $this->result;
            } else {return FALSE;}
...
ORDER BY test_date";
    ...
    * Each array contains the test result data with index keys as
outlined in the <var>$fld_test_param</var> array.
    * @access public
    * @param int Test group id
    * @return mixed adodb record object or boolean
    */
	...
	*/
    ...
    function GetTestsToDo($id=''){
        $this->debug=FALSE;
        if ($this->debug) echo "calling function
GetTestsToDo($id)<br>";
        global $db;
        if(empty($id)) $cond='';
            else $cond="batch_nr='$id'";
        $this->sql="SELECT parameters FROM $this->tb_req_chemlab
WHERE $cond";
        if($this->tparams=$db->Execute($this->sql)){
            if($this->rec_count=$this->tparams->RecordCount()) {
                return $this->tparams;
...
encounter_nr='$this->enc_nr' AND status NOT IN ($this->dead_stat)";
    ...
    *
    * The param $nr takes precedence. If it is not empty it will be
used to find the test parameter.
    * If the $id is needed, set $nr to empty character.
    *
    * The returned adodb record object contains rows of arrays.
    * Each array contain the test data with index keys as outlined
in the <var>$fld_test_param</var> array.
    * @access public
    ...
    */
    function getTestParam($nr=0,$id=''){
        ...
        $this->sql="SELECT * FROM $this->tb_test_param WHERE
$cond";
...
= begin of rows block.
    ...
    */
    function
searchEncounterLabResults($key='',$add_opt='',$limit=FALSE,$len=30,$so=0){
        global $db, $sql_LIKE;
        $debug=FALSE;
        ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
        if(empty($key)) return FALSE;
        $this->sql="SELECT f.encounter_nr,f.send_date,
e.encounter_class_nr, p.pid, p.name_last, p.name_first,
p.date_birth, p.sex
...
$this->res['selr']=$db->SelectLimit($this->sql,$len,$so);
        ...
        if ($this->res['selr']) {
            if
($this->record_count=$this->res['selr']->RecordCount()) {
                # Workaround
...
unsorted result.
    ...
    */
    function
searchLimitEncounterLabResults($key,$len,$so,$sortitem='',$order='ASC'){
        if(!empty($sortitem)){
        ...
        return
$this->searchEncounterLabResults($key,$option,TRUE,$len,$so);
    }
    /**
...
care_test_findings_chemlab where job_id=".$batch_nr;
        ...
        if ($debug) echo "<br>Records for $batch_nr found in table
care_test_request_chemlabor: $num_of_pending_requests<br>";
        ...
        if ($num_of_pending_requests==$num_of_lab_results) {
            if ($debug) echo "all results are given! return code true";
            return 1;
