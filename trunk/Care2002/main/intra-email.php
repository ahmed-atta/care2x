...
href="javascript:gethelp('intramail.php','mail','<?php echo $mode ?>','<?php echo $folder ?>','<?php echo $sendok ?>')"><img <?php echo createLDImgSrc('../','hilfe-r.gif','0','absmiddle') ?> style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)></a><a href="<?php echo $breakfile ?>"><img <?php echo createLDImgSrc('../','close2.gif','0','absmiddle') ?> style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)></a></td>
</tr>
<tr valign=top >
...
<FONT face="Verdana,Helvetica,Arial" size=2>
  &nbsp; <b>';
  if($mode!='listmail') echo '<a href="intra-email.php?sid='.$sid.'&lang='.$lang.'&mode=listmail">'.$LDInbox.'</a> | ';
  	else echo $LDInbox.' | ';
  if($mode!='compose') echo '<a href="intra-email.php?sid='.$sid.'&lang='.$lang.'&mode=compose">'.$LDNewEmail.'</a> | ';
  	else echo $LDNewEmail.' | ';
	echo '<a href="intra-email-addrbook.php?sid='.$sid.'&lang='.$lang.'&mode='.$mode.'&folder='.$folder.'">'.$LDAddrBook.'</a> | 
...
echo '<ul><form name="mailform" action="'.$thisfile.'" method="post" onSubmit="return chkCompose(this)">';
	if(($mode=='sendmail')&&($sendok)) 
	{
		...
		echo '<img '.createMascot('../','mascot1_r.gif','0','bottom').'><font color="#0000ff" size=3> <b>';
	  ...
	  if(!$sendok) 
	  {
	  	echo '
	  			<FONT face="Verdana,Helvetica,Arial" size=2 color="#000080">
						...
						{	
								echo '
	  							 <a href="javascript:useadd(\''.$qa[$i].'\')" title="'.$LDInsertAddr.'" ><img '.createComIcon('../','arrow-blu.gif','0','middle').'> '.$qa[$i].'</a><br>';
						}
		echo '
...
//******************************************* list mail *******************************
 ...
 { 	// prepare inbox for display
	...
	/* Load common icons */
	$img_closemail=createComIcon('../','c-mail.gif','0');
	$img_openmail=createComIcon('../','o-mail.gif','0');
	$img_closefolder=createComIcon('../','cf.gif','0');
	$img_openfolder=createComIcon('../','of.gif','0');
	$img_uparrow=createComIcon('../','arw_up.gif','0');
	$img_dwnarrow=createComIcon('../','arw_down.gif','0');
...
*/
	...
	echo '<img '.$img_openfolder.'> <b>'.$LDInbox.' </b>';
		else echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&mode=listmail&l2h='.$l2h.'"><img '.$img_closefolder.'> '.$LDInbox.'</a>';
	...
	echo '<img '.$img_openfolder.'> <b>'.$LDSent.'</b>';
		else echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&mode=listmail&l2h='.$l2h.'&folder=sent"><img '.$img_closefolder.'> '.$LDSent.'</a>';
	echo '<font size=1 face=verdana,arial color="#0"> (';
	...
	if($folder=='drafts') echo '<img '.$img_openfolder.'> <b>'.$LDDrafts.'</b>';
		else echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&mode=listmail&l2h='.$l2h.'&folder=drafts"><img '.$img_closefolder.'> '.$LDDrafts.'</a>';
	echo '<font size=1 face=verdana,arial color="#0"> (';
	...
	if($folder=='trash') echo '<img '.$img_openfolder.'> <b>'.$LDRecycle.'</b>';
		else echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&mode=listmail&l2h='.$l2h.'&folder=trash"><img '.$img_closefolder.'> '.$LDRecycle.'</a>';
	echo '<font size=1 face=verdana,arial color="#0"> (';
...
*/
	echo '<br>
      ...
      <td valign=top><img src="../gui/img/common/default/pixel.gif" border=0 width=10 height=1>			
	</td>
...
if($maxrow)
{
    ...
    <tr ><td colspan=6 height=1><img src="../gui/img/common/default/pixel.gif" border=0 height=4 width=1></td></tr>
	 <tr bgcolor="#0060ae">
       <td>&nbsp;</td>
	...
	echo '
	   ...
	   if($l2h) echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&l2h=0&mode=listmail&folder='.$folder.'" title="'.$LDSortDate.'"><img '.$img_uparrow; else echo '<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&l2h=1&mode=listmail&folder='.$folder.'" title="'.$LDSortDate.'"><img '.$img_dwnarrow;
	   echo '
...
{
	echo '<img '.createMascot('../','mascot1_r.gif','0','middle').'>
...
{
	echo'<center>
	<img '.createMascot('../','mascot1_r.gif','0','middle').'> 
	<FONT face="Verdana,Helvetica,Arial" size=3 color="#800000">
