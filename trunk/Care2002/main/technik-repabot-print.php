...
{
	window.print()
	window.location.replace("technik-repabot-print.php?<?php echo "sid=$sid&lang=$lang&mode=ack_print&dept=$dept&tdate=$tdate&ttime=$ttime&tid=$tid"; ?>")
}
function move2arch()
{
	...
	window.location.replace("technik-repabot-print.php?<?php echo "sid=$sid&lang=$lang&mode=archive&dept=$dept&tdate=$tdate&ttime=$ttime&tid=$tid"; ?>&clerk="+c)
}
function parentref(n)
{
    if(n==1) window.opener.location.replace("technik-repabot.php?sid=<?php echo "$sid&lang=$lang&userck=$userck"?>&cat=<?php echo $cat ?>&nofocus="+n+"&showlist=1");
    else window.opener.location.replace("technik-repabot.php?sid=<?php echo "$sid&lang=$lang&userck=$userck"?>&cat=<?php echo $cat ?>&showlist=1");
    //
	<?php
	if($statseen || $deltodo)
	{
...
{
   echo'<img '.createMascot('../','mascot1_r.gif','0','absmiddle').'>'.$LDNoDataFound;
}
?>
