		  ...
		  switch($mode)
		  {
		     case 'update':
							      $sql="UPDATE care_test_request_".$db_request_table." SET 
                                   lang = '".$lang."', status = 'received', blood_pb = '".htmlspecialchars($blood_pb)."', blood_rb = '".htmlspecialchars($blood_rb)."', blood_llrb = '".htmlspecialchars($blood_llrb)."', 
								   ...
								   receipt_ack = '".htmlspecialchars($receipt_ack)."', mainlog_nr = '".htmlspecialchars($mainlog_nr)."', lab_nr = '".htmlspecialchars($lab_nr)."', mainlog_date = '".formatDate2STD($mainlog_date,$date_format)."', lab_date = '".formatDate2STD($lab_date,$date_format)."', 
								   mainlog_sign = '".htmlspecialchars($mainlog_sign)."', lab_sign = '".htmlspecialchars($lab_sign)."', modify_id = '".$HTTP_COOKIE_VARS[$local_user.$sid]."'
		     ...
		     case 'done':
							      $sql="UPDATE care_test_request_".$db_request_table." SET status = 'done'  WHERE batch_nr = '".$batch_nr."'";
...
<FONT  COLOR="<?php echo $cfg['top_txtcolor']; ?>"  SIZE=+2  FACE="Arial"><STRONG><?php echo $LDPendingTestRequest; if($batchrows) echo " (#".$stored_request['batch_nr']." ".$stored_request['room_nr']." ".$stored_request['dept'].")"; ?></STRONG></FONT>
</td>
...
<a href="javascript:document.form_test_request.submit()"><img <?php echo createLDImgSrc('../','savedisc.gif','0') ?> alt="<?php echo $LDSaveEntry ?>"></a>
<a href="javascript:printOut()"><img <?php echo createLDImgSrc('../','printout.gif','0') ?> alt="<?php echo $LDPrintOut ?>"></a>
<?php
if ($stored_request['release_via']!='' && $stored_request['mainlog_sign']!='' && $stored_request['status']!='done')
{
?>
<a href="<?php echo $thisfile."?sid=".$sid."&lang=".$lang."&edit=".$edit."&mode=done&target=".$target."&subtarget=".$subtarget."&batch_nr=".$batch_nr."&user_origin=".$user_origin."&noresize=".$noresize; ?>"><img <?php echo createLDImgSrc('../','done.gif','0') ?> alt="<?php echo $LDDone ?>"></a>
<?php
}
?>
...
{
?>
<img <?php echo createMascot('../','mascot1_r.gif','0','absmiddle') ?>><font size=3 face="verdana,arial" color="#990000"><b><?php echo $LDNoPendingRequest ?></b></font>
<p>
