...
echo '
  ...
  switch($folder)
  {
  	case 'inbox': if($HTTP_COOKIE_VARS[$local_user.$sid]!=$content['sender']) 
							{ $inp=0; echo '<input type="button" value="'.$LDReply.'" onClick="submitForm(1)"> '; break; }
								else echo '<input type="button" value="'.$LDBack.'" onClick="window.location.href=\'intra-email.php?sid='.$sid.'&lang='.$lang.'&mode=listmail&folder=inbox&l2h='.$l2h.'\'"> '; break;
  	case 'sent': if($HTTP_COOKIE_VARS[$local_user.$sid]!=$content['recipient'])
							{ $inp=1; echo '<input type="button" value="'.$LDResend.'" onClick="submitForm(2)"> ';break; }
								else  echo '<input type="button" value="'.$LDBack.'" onClick="window.location.href=\'intra-email.php?sid='.$sid.'&lang='.$lang.'&mode=listmail&folder=sent&l2h='.$l2h.'\'"> '; break;
 ...
 echo '
	 &nbsp; &nbsp; &nbsp; <a href="javascript:printer_v()" title="'.$LDClk4printer.'">'.$LDPrinterVersion.' <img '.createComIcon('../','bul_arrowgrnsm.gif','0','bottom').'></a>	
