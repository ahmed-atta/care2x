...
if($link&&$DBLink_OK)  
{	
		$sql='SELECT * FROM care_users WHERE login_id="'.addslashes($itemname).'"';
						$ergebnis=mysql_query($sql,$link);
						if($ergebnis)
							{	$zeile=mysql_fetch_array($ergebnis);
								if ($finalcommand=='changelock')
								{	if ($zeile['lockflag']) $newlockflag=0; else $newlockflag=1;
									$sql='UPDATE care_users SET lockflag='.$newlockflag.' WHERE login_id="'.$itemname.'"';	
									if (mysql_query($sql,$link))
									{
							        header("Location: edv_user_access_list.php?sid=$sid&lang=$lang&remark=lockchanged"); exit;
...
<?php if($cfg['dhtml'])echo'<a href="javascript:window.history.back()"><img '.createLDImgSrc('../','back2.gif','0').'  style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)>';?></a><a href="javascript:gethelp('edp.php','access','lock','<?php echo $zeile['lockflag'] ?>')"><img <?php echo createLDImgSrc('../','hilfe-r.gif','0') ?>  <?php if($cfg['dhtml'])echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)>';?></a><a href="<?php echo $breakfile;?>"><img <?php echo createLDImgSrc('../','close2.gif','0') ?> alt="<?php echo $LDClose ?>"  <?php if($cfg['dhtml'])echo'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)>';?></a></td>
</tr>
<tr>
