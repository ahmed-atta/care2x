...
require_once('../include/inc_intramail_domains.php');
		...
		if(($mode=='access')&&($password!='')&&($username!='')&&($dept!='')) 
			{
				...
				{ 
					...
					if($rows==1)
					{
						...
						//echo "$content[email]<br>";echo "$content[pw]<br>";
						//echo crypt($password,substr($content[pw],0,2));// if last check time stamp found check for  new mails
...
href="javascript:gethelp('intramail.php','pass','<?php echo $newuser ?>')"><img <?php echo createLDImgSrc('../','hilfe-r.gif','0','absmiddle') ?> style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)></a><a href="<?php echo $breakfile ?>"><img <?php echo createLDImgSrc('../','close2.gif','0','absmiddle') ?> style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)></a></td>
</tr>
<tr valign=top >
...
<p><br><ul>
<?php if($onError!="") echo '
	<img '.createMascot('../','mascot1_r.gif','0','bottom').'><FONT face="Verdana,Helvetica,Arial" size=2 color="#800000"> '.$onError.'</font>';
?>
...
<table border=0>
  <tr>
    <td><img <?php echo createMascot('../','mascot1_r.gif','0','bottom') ?>></td>
...
<?php if (!$newuser)echo '
<p><br>
<a href="'.$thisfile.'?sid='.$sid.'&lang='.$lang.'&newuser=1">'.$LDNewReg.' <img '.createComIcon('../','bul_arrowgrnsm.gif','0','bottom').'></a>
';
?>
  </ul>
 </table>
