...
<td width="20">
<?php if(!$showonly) : ?>
<img <?php echo $img_delete ?> alt="<?php echo $LDReset ?>" onClick="javascript:document.icd10.reset()">
<?php endif ?>
</td>
...
function drawdata(&$data,&$advdata)
{
	global $toggle,$parentcode,$grandcode,$priocolor,$LDInclusive,$LDExclusive,$LDNotes,$LDRemarks,$LDExtraCodes,$LDAddCodes;
 	global $idx,$parenthref,$showonly,$parentdata,$img_arrow, $img_warn, $img_info, $img_bubble, $img_blue, $img_t2, $img_plus, $img_reset;
						echo "
						<tr bgcolor=";
						...
						if($data[inclusive]) 
						{
							echo '&nbsp;<a href="javascript:ssm(\'i_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_arrow.' alt="'.$LDInclusive.'"></a>';
						...
						if($data[exclusive])
						{
							 echo '&nbsp;<a href="javascript:ssm(\'e_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_warn.' alt="'.$LDExclusive.'"></a>';
							drawAdditional("e",$data[diagnosis_code],$data[exclusive],"ffccee",$LDExclusive);
						}
						if($data[notes])
						{
							echo '&nbsp;<a href="javascript:ssm(\'n_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_info.' alt="'.$LDNotes.'"></a>';
							drawAdditional("n",$data[diagnosis_code],$data[notes],"ffcc99",$LDNotes);
						}
						if($data[remarks]) 
						{
							echo '&nbsp;<a href="javascript:ssm(\'r_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_bubble.' alt="'.$LDRemarks.'"></a>';
							drawAdditional("r",$data[diagnosis_code],$data[remarks],"cceeff",$LDRemarks);
						}
						if($data[extra_codes])
						{
						 	echo '&nbsp;<a href="javascript:ssm(\'x_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_plus.' alt="'.$LDExtraCodes.'"></a>';
							drawAdditional("x",$data[diagnosis_code],$data[extra_codes],"ffff66",$LDExtraCodes);
						}
						if($data[extra_subclass])
						{
							echo '&nbsp;<a href="javascript:ssm(\'s_'.cleandata($data[diagnosis_code]).'\');"><img '.$img_reset.' alt="'.$LDAddCodes.'"></a>';
