...
$thisfile='pflege-station-patientdaten-doconsil-chemlabor.php';
							  ...
							  if(prepareTestElements())
							  {
							     $sql="INSERT INTO care_test_request_".$db_request_table." 
							      ...
							      if($ergebnis=mysql_query($sql,$link))
									...
									//echo $sql;
									mysql_close($link);
									 header("location:labor_test_request_aftersave.php?sid=$sid&lang=$lang&edit=$edit&saved=insert&pn=$pn&station=$station&user_origin=$user_origin&status=$status&target=chemlabor&noresize=$noresize&batch_nr=$batch_nr");
									 exit;
								  }
								  else 
								  {
							  ...
							  if(prepareTestElements())
							  {
							     $sql="UPDATE care_test_request_".$db_request_table."  SET
							      ...
							      if($ergebnis=mysql_query($sql,$link))
									...
									//echo $sql;
									mysql_close($link);
									 header("location:labor_test_request_aftersave.php?sid=$sid&lang=$lang&edit=$edit&saved=update&pn=$pn&station=$station&user_origin=$user_origin&status=$status&target=chemlabor&batch_nr=$batch_nr&noresize=$noresize");
									 exit;
								  }
								  else
								   {
...
function printOut()
{
	urlholder="labor_test_request_printpop.php?sid=<?php echo $sid ?>&lang=<?php echo $lang ?>&user_origin=<?php echo $user_origin ?>&subtarget=chemlabor&batch_nr=<?php echo $batch_nr ?>&pn=<?php echo $stored_request['patnum'] ?>";
...
<table border=0>
  <tr>
    ...
    <td valign="bottom"><img <?php echo createComIcon('../','angle_down_r.gif','0') ?>></td>
  </tr>
</table>
<?php
}
    ...
    while(list($x,$v)=each($LD_Elements))
	{
	  ...
	  else
	  {
		 ...
		 { 
		   ...
		   {
		      echo '
			          <td align="right" '.$tdbgcolor.'><img '.createComIcon('../','violet_phone.gif','0','absmiddle').'></td>';
	        }
			else
			{ 
		      echo '
...
?>
  <tr>
    ...
    <td colspan=11><input type="text" name="add_notes" size=65 maxlength=60 value="<?php if($edit_form||$read_form) echo stripslashes($stored_request['notes']); ?>"></td>
  </tr>
  <tr>
