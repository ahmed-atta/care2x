...
<? if($saveok) : ?>
 ...
 window.opener.parent.location.replace('drg-composite-start.php?sid=<?="$ck_sid&lang=$lang&pn=$pn&opnr=$opnr&ln=$ln&fn=$fn&bd=$bd&dept=$dept&oprm=$oprm&display=composite" ?>');
 window.close();
</script>
	<? exit; ?>
<? endif ?>
							...
							if($ergebnis=mysql_query($sql,$link))
       						{
								...
								if($rows)
								{
									...
									if(trim($pdata[ops_code]!="")) $linebuf=trim($pdata[ops_code])."~".$linebuf;
								}
							}
    				$sql="UPDATE $dbtable SET ops_code='$linebuf' WHERE op_nr='$opnr' AND patnum='$pn' AND dept='$dept' AND op_room='$oprm'";
			}
			else
			{
							// first get the last op number
		 					$sql="SELECT op_nr FROM $dbtable WHERE dept='$dept'	AND op_room='$oprm' ORDER BY op_nr DESC";
							//print $sql;
							if($ergebnis=mysql_query($sql,$link))
       						{
								$rows=0;
    							...
    							$sql="INSERT INTO $dbtable (op_nr,patnum,lastname,firstname,bday,dept,op_room,ops_code) 
											VALUES ('$opnr','$pn','$ln','$fn','$bd','$dept','$oprm','$linebuf')";
							}
				...
				if($ergebnis)
       			{
					header("location:$thisfile?sid=$ck_sid&lang=$lang&saveok=1&pn=$pn&opnr=$opnr&ln=$ln&fn=$fn&bd=$bd&dept=$dept&oprm=$oprm&display=$display");
					//print $sql;
					exit;
				}
