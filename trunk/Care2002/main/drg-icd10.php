...
if (!$sid||($sid!=$ck_sid)||!$pn) {header("Location:../language/".$lang."/lang_".$lang."_invalid-access-warning.php"); exit;}; 
...
{
	urlholder="drg-icd10-search.php?sid=<?="$ck_sid&lang=$lang&pn=$pn&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&dept=$dept&oprm=$oprm&display=$display" ?>&keyword="+k+"&showonly="+x;
...
function deleteItem(i)
{
	...
	{
		window.location.href='drg-icd10.php?sid=<?="$ck_sid&lang=$lang&mode=delete&pn=$pn&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&dept=$dept&oprm=$oprm&display=$display" ?>&item='+i;
	}
}
...
<b><?=$LDIcd10 ?></b></font>&nbsp;
<!--  <input type="button" value="<?=$LDSearch4ICD10 ?>" onClick="javascript:openICDsearch('','0')">&nbsp;
 <input type="button" value="<?=$LDQuickList ?>" onClick="javascript:openICDsearch('','0')">
 -->
<? endif ?>
  ...
  <tr>
	...
	<table border=0 cellpadding=1 cellspacing=1 width=100%> 
		<tr bgcolor="#0000aa">
 		...
 		<td ><font face=arial size=2 color=#ffffff><b><?=$LDMainAuxDiag ?></b></td>
 		<td ><font face=arial size=2 color=#ffffff><b><?=$LDLocalization ?></b></td>
 		<td ><font face=arial size=2 color=#ffffff><b><?=$LDDiagBy ?></b></td>
<? if($display=="composite") : ?>
...
?>
	...
	<input type="button" value="<?=$LDSearch ?>" onClick="javascript:openICDsearch('','0')">&nbsp;
 	<p><input type="button" value="<?=$LDQuickList ?>" onClick="javascript:openQuicklist('icd10')"><p>
	</td>
	<? endif ?>
  </tr>
</table>
