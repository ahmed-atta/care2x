...
if($order_nr&&$dept)
{
  ...
  if($link&&$DBLink_OK) 
  {
	...
	{
		if($rows=mysql_num_rows($ergebnis))  $content=mysql_fetch_array($ergebnis);
	}
	else
	{ 
...
{
	this.print()
	this.location.replace("products-bestellbot-print.php?<?php echo "sid=$sid&lang=$lang&userck=$userck&mode=ack_print&cat=$cat&dept=$dept&order_nr=$order_nr&status=$status"; ?>");
}
function move2arch()
{
	...
	this.location.replace("products-bestellbot-print.php?<?php echo "sid=$sid&lang=$lang&userck=$userck&mode=archive&cat=$cat&dept=$dept&order_nr=$order_nr&status=$status&clerk="; ?>"+c);
}
function parentref(n)
{
    if(n==1) window.opener.location.replace("products-bestellbot.php?sid=<?php echo "$sid&lang=$lang&userck=$userck"?>&cat=<?php echo $cat ?>&nofocus="+n+"&showlist=1");
    else window.opener.location.replace("products-bestellbot.php?sid=<?php echo "$sid&lang=$lang&userck=$userck"?>&cat=<?php echo $cat ?>&showlist=1");
    //
	<?php
	if($statseen || $deltodo)
	{
...
{
    echo' <img '.createMascot('../','mascot1_r.gif','0','middle').'>'.$LDDataNoFoundTxt.$sql;
}
?>
<form name="opt4">
