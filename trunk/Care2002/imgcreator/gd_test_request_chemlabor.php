...
if (eregi('inc_test_request_gd_chemlabor.php',$PHP_SELF)) 
...
if(file_exists('../language/'.$lang.'/lang_'.$lang.'_konsil_'.$subtarget.'.php')) include_once('../language/'.$lang.'/lang_'.$lang.'_konsil_'.$subtarget.'.php');
  else include_once('../language/'.LANG_DEFAULT.'/lang_'.LANG_DEFAULT.'_konsil_'.$subtarget.'.php');
...
$mark_v_offset=1; /* Vertical offset of the marking blocks */
    ...
    if(file_exists('../gui/img/common/default/violet_phone2.png'))
	{
	   $v_phone = ImageCreateFrompng('../gui/img/common/default/violet_phone2.png');
	}
	...
	{
	   $lab_bc = ImageCreateFrompng('../cache/barcodes/lab_'.$batch_nr.'.png');
	}
   ...
   $sx=$left_border+($hdiv*4);
	...
	for($n=0;$n<8;$n++)
	{
       ...
       for($i=0,$sx=($left_border+($hdiv*30));$i<10;$i++,$sx=$sx+$hdiv)
	   {
			  ...
			  if(substr($result['patnum'],$n,1)==$i) doFilledBlock();
			  else doBlock();
        }
	}
