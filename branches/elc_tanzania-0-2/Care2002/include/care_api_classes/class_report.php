...
class report extends core {
  ...
  */  
  function SetReportingTable($tbl='') {
    ...
    if (empty($tbl)) {
      if (empty($tbl)) {
        $this->sql="CREATE TEMPORARY TABLE $this->coretable (id int unsigned not null, primary key (id)) TYPE=HEAP";
      } else {
        $this->sql="CREATE TEMPORARY TABLE $this->coretable TYPE=HEAP SELECT $result_fields FROM $tbl ";
      }
    } else {
      $this->sql="CREATE TEMPORARY TABLE $this->coretable TYPE=HEAP SELECT $result_fields FROM $tbl ";
    }
  ...
  */
  function SetReportingLink($tbl1,$tbl1_key, $tbl2,$tbl2_key) {
    ...
    if ( ! (empty($tbl1) || empty($tbl1_key) || empty($tbl2) || empty($tbl2_key)) ) {
      ...
      $this->setTable($this->tmp_tbl_name.=time());
      $this->sql="CREATE TEMPORARY TABLE $this->coretable TYPE=HEAP SELECT $result_fields FROM $tbl1 INNER JOIN $tbl2 ON $tbl1.$tbl1_key=$tbl2.$tbl2_key ";
      return ($this->Transact($this->sql)) ? $this->coretable : FALSE;
    } else {
      return FALSE;
    }
  ...
  */
  function TMP_GroupBy($table,$field) {
    ...
    $result_fields = $this->_SetColumnNamesAsString($table,$this->GetFieldnames($table));
      $this->setTable($this->tmp_tbl_name.=time());
      $this->sql="CREATE TEMPORARY TABLE $this->coretable TYPE=HEAP SELECT $result_fields FROM $table GROUP BY $field ";
    return ($this->Transact($this->sql)) ? $this->coretable : FALSE;
