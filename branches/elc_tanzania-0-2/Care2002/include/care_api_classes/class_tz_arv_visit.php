...
//---------------------------------------------------------------------------------------------------	
	...
	//reads the aids defining events of a patient from the db
		global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	//reads the Reason for changing ARV status from the DB
		global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	function insertNewVisit($values,$a_item_no,$r_item_no) {
	    ...
	    $this->set_visit_data($values,$a_item_no,$r_item_no);
			...
			if(!$this->Transact($this->sql_1)) {
				if ($db->ErrorNo()==1062) {
					$this->error_message['db']=sprintf($this->msg_tpl_big,"There is already a visit for this encounter");		
				}
				else {
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------	
	...
	//reads the ARV drugs prescribed in the pharmacy module for the current encounter
	    global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	function displaySelectedARVDrugs_String() {
	    global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	//reads all drugs prescribed to the current encounter of a patient in the pharmacy module
		global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------	
	...
	//returns an array containing the lab values with their associated information like test_time, unit...
	    global $db;
		$debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	/* returns an array that include only the abnormal lab results.
	 * Abnormal lab results are defined as results that lies outside the higher and lower boundary that 
	 * can be entered into Laboratory->Administration->Test Parameters
	**/
	...
	/* $param_name can be the name of an laboratory test parameter for example HB
	 * return value is an array with the lab value and it's associated information like unit,test_time..' 
	 * */ 
	...
	// displays and table with all Abnormal Labresults
		global $date_format;
	...
	/* $param_name: name of the lab value for example CD4
	 * returns an table displaying the lab value
	 */
		global $date_format;
...
//---------------------------------------------------------------------------------------------------------------------
	...
	// method for for filling the select box in the AIDS defining events form of the ARV Module
    	global $db;
   		$debug=false;
        ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
  	...
  	/* method for for filling the select box in the ARV status reason form of the ARV Module
  	 * Parameter $status_code: $_GET[arv_data][care_tz_arv_status_id] of the ARV visit form 
  	 */
    	global $db;
   		$debug=false;
        ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
...
//-------------------------------------------------------------------------------------------------------
    ...
    // method for for filling the select box in the AIDS defining events form of the ARV Module
  	...
  	// method for for filling the select box in the ARV reason form of the ARV Module
    	$counter=0;
...
//---------------------------------------------------------------------------------------------------------------
	...
	// method for for filling the item list in the AIDS defining events form of the ARV Module
	    global $db;
	    $debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
    ...
    // method for for filling the item list in the ARV Status reason form of the ARV Module
	    global $db;
	    $debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
...
//---------------------------------------------------------------------------------------------------------------------	
	function displaySelectedAIDSEvents_Items($array){
	  // method for for filling the selected item list in the ARV AIDS defining events form of the ARV Module
		$counter=0;
	...
	/* method for for filling the selected item list in the ARV AIDS defining events form 
	 * and the ARV status reason form of the ARV module
	 * Parameter $array: include the $_GET array of the selected items of teh item list
	 */
...
//---------------------------------------------------------------------------------------------------------------------
	...
	//displays an table of the selected items from the AIDS defining Events form
   		global $db;
	    $debug=false;
	    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
	...
	//displays an table of the selected items from the ARV Status reason form
	    $table_string="<table>\n";
	    while (list($x,$v) = each($r_item_no)) {
    ...
    //displays an table of the selected items from the ARV status reason form or the AIDS defining events form
...
//----------------------------------------------------------------------------------------------------------------------------
	...
	//method for creating a querystring
		global $HTTP_GET_VARS;
		reset ($HTTP_GET_VARS); 
		#stripslashes($_GET[$name]);
		$querystring="";
		$querystring=$querystring."&arv_visit_id=".$_GET['arv_visit_id']."&pid=".$_GET['pid']."&mode=".$_GET['mode']."&encounter_nr=".$_GET['encounter_nr'];
