...
class Diagnostics extends Encounter {
    ...
    while(list($x,$v) = each($dataarray))
    {
    	if(strstr($x,"diagnosistype"))
    	{
    		...
    		if($dataarray['diagnosistype_'.$form_id]=="revisit")
    			$return = $this->insert_diagnosis_as_revisit($dataarray['parentcase_'.$form_id], $dataarray['pid'], $dataarray['encounter'], $dataarray['icd_'.$form_id], $dataarray['icddesc_'.$form_id], $dataarray['comment_'.$form_id], $dataarray['diagnosistype_'.$form_id]);
    		else
    			$return = $this->insert_diagnosis_as_new($dataarray['pid'], $dataarray['encounter'], $dataarray['icd_'.$form_id], $dataarray['icddesc_'.$form_id], $dataarray['comment_'.$form_id], $dataarray['diagnosistype_'.$form_id]);
  ...
  function insert_diagnosis_as_new($pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, $type) {
    global $db;
    $debug=false;
    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
  ...
  function insert_diagnosis_as_revisit($parent_case_nr, $pid, $encounter_nr, $ICD10_code, $ICD10_description, $comment, $type) {
    global $db;
    $debug=FALSE;
    ($debug) ? $db->debug=TRUE : $db->debug=FALSE;
  ...
  */
  ...
  function Display_Quicklist_Elements_Items($id){
    if (empty($id) || $id==-1)
    {
      echo '<option value="-1">Nothing found! Select a quicklist!</opion>';
      return false;
    }
    ...
    if(!$counter) echo '<option value="-1">Nothing found, try fuzzy search!</opion>';
    return true;
  }
  //------------------------------------------------------------------------------
  ...
  function Display_Selected_Diagnoses($encounter, $itemlist, $pid){
    ...
    echo '<form action="" method="post">
          ...
          <tr bgcolor="#99ccff">
            ...
            <td width="10%"><div align="center">new patient</div></td>
            <td width="10%"><div align="center">new case</div></td>
            <td width="10%"><div align="center">revisit</div></td>
            <td width="30%"><div align="center">comment</div></td>
          </tr>';
    while(list($x,$v) = each($itemlist))
    {
    	...
    	if($this->icd_old_array[$v])
    	{
    		...
    		$oldcase_code = '<br><font color="#FF0000">Same diagnose: '.date("Y-m-d - H:i:s",$case_arr['timestamp']).'</font> - <a href="javascript:openpopup(\'icd10_showdetails.php\',\'show_details\',\'id\',\''.$this->icd_old_array[$v].'\');">VIEW</a>
  	...
  	echo '       
  		      <tr bgcolor="#99ccff">
            <td width="40%"><input type="reset" value="Reset fields"></td>
            <td width="10%"><div align="center">new patient</div><input type="hidden" name="todo" value="submit"></td>
            <td width="10%"><div align="center">new case</div><input type="hidden" name="encounter" value="'.$encounter.'"></td>
            <td width="10%"><div align="center">revisit</div><input type="hidden" name="pid" value="'.$pid.'"></td>
            <td width="30%" align="right"><input type="submit" value="Submit diagnose"></td>
          </tr>
       </table>                
 	  </form>';
  }
  ...
  function Display_Archived_Diagnoses($pid){
    ...
    echo '<form action="" method="post">
          ...
          <tr bgcolor="#99ccff">
						...
						<td width="5%"><div align="center">Case</div></td>
            <td width="38%">Diagnosis</td>
    ...
    while(list($x,$v) = each($this->icd_old_array))
    {
    	$idcounter++;
  		...
  		if($case_arr['parent_case_nr']!=-1 && $case_arr['parent_case_nr'] && $case_arr['type']=="revisit" )
  		{
  			...
  			$case_old = $this->GetCase($case_arr['parent_case_nr']);
				$oldcase_code = '<br><font color="#FF0000">Revisit of Case '.$case_arr['parent_case_nr'].' from '.date("Y-m-d",$case_old['timestamp']).'</font> - <a href="javascript:openpopup(\'icd10_showdetails.php\',\'show_details\',\'id\',\''.$case_arr['parent_case_nr'].'\');">SHOW</a>';
  		}
  		else
  			$oldcase_code = "";
  		$revisit="checked";
  		$new="";
            ...
            if(!$case_arr['comment']) echo '--- no comment available ---';
  	...
  	echo '       
  		    <tr bgcolor="#99ccff">
						...
						<td><div align="center">Case</div></td>
            <td>Diagnosis  </td>
