...
<table width="100%" border="0">
	  <tr>
		...
		<td width="17%" class="adm_input"><div align="center"><input type="button" name="show" value="Search" onClick="javascript:submit_form(this,'icd10_search.php','<?php echo $sid ?>','')">
		</div></td>
		...
		<td width="38%" bgcolor="#CAD3EC"><div align="center"><input type="button" name="show" value="Manage Quicklist" onClick="javascript:submit_form(this,'icd10_manage.php','<?php echo $sid ?>','')">
		</div></td>
	  </tr>
	</table>
         ...
         <table>
          <tr> 
            <td colspan="3" class="adm_input"> <div align="right"> 
                Enter new Quicklist name:</div></td>
            <td class="adm_input"> 
                <input name="createquicklist" type="text" size="30" maxlength="30">
                <input type="button" name="AddNewQuicklist" value="CREATE" onClick="javascript:submit_form(this,'icd10_manage.php','<?php echo session_id();?>','createquicklist');">
            </td>
          </tr>
          <tr> 
            <td colspan="3" class="adm_input"> <div align="right"> Select an existing 
                Quicklist:</div></td>
            <td class="adm_input"> 
<select name="quicklist" onChange="submit_form(this,'<?php echo $thisfile;?>','<?php echo session_id();?>','manage');">
                  <option selected value="-1"> Please select a quicklist item: </option>
<?php $diagnostic_obj->Display_Quicklist_Elements($quicklist); ?>
                  ...
                  <table border="0" cellpadding="0" cellspacing="0">
                  	...
                  	<tr>
                  		<td>
                  			<input <?php if(!$quicklist) echo 'disabled'; ?> type="radio" value="exact" name="search_mode"<?php if(!$search_mode || $search_mode=="exact") echo "checked"; ?>> exact search
                  		</td><td align="right">
                  			<input <?php if(!$quicklist) echo 'disabled'; ?> type="radio" value="fuzzy" name="search_mode"<?php if($search_mode=="fuzzy") echo "checked"; ?>> fuzzy search
                  		</td>
                  	</tr>
                  </table>
                  <br>
