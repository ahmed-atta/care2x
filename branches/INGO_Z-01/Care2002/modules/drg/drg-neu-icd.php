...
switch($mode)
{
	case 'delete': 
	{				
		// Elpidio: if($DRG_obj->deleteDiagnosis($itemx)){
		// delete := status='deleted' because of data security, not DELETE
    $sql="UPDATE care_diagnosis_encounter 
					SET status='deleted', history=CONCAT(history,'Delete ".date('Y-m-d H:i:s')." ".$HTTP_SESSION_VARS['sess_user_name']."\n'),
			    modify_id='".$HTTP_SESSION_VARS['sess_user_name']."'			
			    WHERE diagnosis_nr=$itemx";
    /* direkt löschen $sql="DELETE FROM care_diagnosis_encounter WHERE diagnosis_nr=$itemx AND encounter_nr=$pn"; */
		if($erg=&$db->Execute($sql)){
			header($saved_header);
			exit;
		}
		break;
	...
	{				
		...
		$sql="UPDATE care_diagnosis_encounter
 					SET category_nr=$val, modify_id='".$HTTP_SESSION_VARS['sess_user_name']."'
					WHERE diagnosis_nr=$itemx AND encounter_nr=$pn";						 
		if($erg=&$db->Execute($sql)){
			header($saved_header);
			exit;
		}
	...
	{				
		...
		$sql="UPDATE care_diagnosis_encounter
		      SET localization=$val, modify_id='".$HTTP_SESSION_VARS['sess_user_name']."'
					WHERE diagnosis_nr=$itemx AND encounter_nr=$pn";
		if($erg=&$db->Execute($sql)){			
			header($saved_header);
			exit;
...
/* nicht mehr nötig if($display=='composite'){
...
{
	urlholder="drg-icd10-search-neu.php?sid=<?php echo "$sid&lang=$lang&pn=$pn&edit=$edit&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&group_nr=$group_nr&dept_nr=$dept_nr&oprm=$oprm&display=$display" ?>&keyword="+k+"&showonly="+x;
...
function deleteItem(i)
{
	...
	{
		//window.location.href='drg-neu-icd.php?sid=<?php echo "$sid&lang=$lang&mode=delete&pn=$pn&edit=$edit&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&group_nr=$group_nr&dept_nr=$dept_nr&oprm=$oprm&display=$display" ?>&item='+i;
...
{
	urlholder="drg-quicklist-neu.php?sid=<?php echo "$sid&lang=$lang&pn=$pn&edit=$edit&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&group_nr=$group_nr&dept_nr=$dept_nr&oprm=$oprm" ?>&target="+t;
	drgwin_<?php echo $uid ?>=window.open(urlholder,"drgwin_<?php echo $uid ?>","width=600,height=450,menubar=no,resizable=yes,scrollbars=yes");
	//window.drgwin_<?php echo $uid ?>.moveTo(100,100);
}

function openPatientlist(t)
{
	urlholder="drg-patientlist.php?sid=<?php echo "$sid&lang=$lang&pn=$pn&edit=$edit&ln=$ln&fn=$fn&bd=$bd&opnr=$opnr&group_nr=$group_nr&dept_nr=$dept_nr&oprm=$oprm" ?>&target="+t;
	drgwin_<?php echo $uid ?>=window.open(urlholder,"drgwin_<?php echo $uid ?>","width=600,height=450,menubar=no,resizable=yes,scrollbars=yes");
	//window.drgwin_<?php echo $uid ?>.moveTo(100,100);
}
// -->
...
<?php 
$enc=$DRG_obj->GetBasic4Data(); $encounter=$enc->FetchRow(); echo $encounter['name_last'].", ".$encounter['name_first']."   ".formatDate2Local($encounter['date_birth'],$date_format);
...
<a href="javascript:window.history.back()" ><img <?php echo createLDImgSrc($root_path,'back2.gif','0') ?> align="right"></a><br>
...
#nicht mehr nötig: if (is_object($drg)) {
	...
	$sql="SELECT d.code, d.diagnosis_nr, e.*, c.description, m.description AS parent_desc,
				...
				LEFT OUTER JOIN care_category_diagnosis AS cat ON e.category_nr=cat.nr
				LEFT OUTER JOIN care_type_localization AS loc ON e.localization=loc.nr
				WHERE e.diagnosis_nr=d.diagnosis_nr AND d.code=c.diagnosis_code AND e.encounter_nr=$pn AND (e.status!='deleted' OR IsNull(e.status))
				ORDER BY e.category_nr ASC, e.date DESC";
													 	
	$res=&$db->Execute($sql);
	if (!$res) echo $db->ErrorMsg();
 
	while($icd=$res->FetchRow()){
		# Start creating the display
		if($icd['category_nr']=="1") $fcolor="#0000ff"; else $fcolor="#000000";
		echo "<tr bgcolor=";
		if($toggle) { echo "#e0e0e0>"; $toggle=0;} else {echo "#ffffff>"; $toggle=1;};
		echo '
...
?>
	...
	<p><input type="button" value="<?php echo $LDPatientList ?>" onClick="javascript:openPatientlist('diagnosis')"><p>
	<!-- Code von IRZ -->	
	<a href="javascript:window.parent.close()" ><img <?php echo createLDImgSrc($root_path,'close2.gif','0') ?>></a>
	</td>
	<?php } ?>
  </tr>
</table>
