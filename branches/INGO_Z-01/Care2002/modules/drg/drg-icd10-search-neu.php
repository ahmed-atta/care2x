...
<script language="javascript" >
	window.opener.location.replace('drg-neu-icd.php?sid=<?php echo "$sid&lang=$lang&pn=$pn&opnr=$opnr&edit=$edit&ln=$ln&fn=$fn&bd=$bd&group_nr=$group_nr&dept_nr=$dept_nr&oprm=$oprm&y=$y&m=$m&d=$d&display=composite&newsave=1"; ?>');
...
$toggle=0;
$thisfile='drg-icd10-search-neu.php';
...
{
	//window.location.href='drg-icd10-search-neu.php?sid=<?php echo "sid=$sid&lang=$lang&pn=$pn&opnr=$opnr&edit=$edit&ln=$ln&fn=$fn&bd=$bd&dept_nr=$dept_nr&oprm=$oprm&display=$display" ?>&keyword='+k;
...
<ul>
<FORM action="drg-icd10-search-neu.php" method="post" name="searchdata" onSubmit="return pruf(this)">
...
function drawdata(&$data,&$advdata)
{
	global $toggle,$parentcode,$grandcode,$priocolor,$LDInclusive,$LDExclusive,$LDNotes,$LDRemarks,$LDExtraCodes,$LDAddCodes;
 	global $idx,$parenthref,$showonly,$parentdata,$img;
						...
						elseif($toggle) { echo "#e0e0e0>"; $toggle=0;} else {echo "#ffffff>"; $toggle=1;};
						echo '<td>';
...
<p>
<FORM action="drg-icd10-search-neu.php" method="post" onSubmit="return pruf(this)" name="form2">
