...
switch($target){
	...
	case 'diagnosis':
							...
							$rowcolor="#0000aa";
							$element="icd_code";
							$searchfile="drg-icd10-search-neu.php";
							$save_related=1;
...
$thisfile="drg-patientlist.php";

if($mode=='save')
{
	$itemselector='sel';
	include($root_path.'include/inc_drg_entry_save_neu.php');
}else{
	#if($qlist_obj=$DRG_obj->DeptQuicklist($target,$dept_nr)){
	#	$linecount=$DRG_obj->LastRecordCount();

	$sql="SELECT d.code, d.code_parent, c.diagnosis_code AS nr, c.description, p.description AS parent_desc FROM care_diagnosis AS d
        INNER JOIN care_encounter AS e ON e.pid=d.pid	  
				LEFT OUTER JOIN care_icd10_de AS c ON d.code=c.diagnosis_code
				LEFT OUTER JOIN care_icd10_de AS p ON d.code_parent=p.diagnosis_code				 
				WHERE e.encounter_nr=$pn ORDER BY d.create_time DESC";
	$qlist_obj=&$db->Execute($sql);
	if (!$qlist_obj) echo $db->ErrorMsg(); else $linecount=$qlist_obj->RecordCount();
}
?>
...
<HEAD>
<?php echo setCharSet(); ?>
 <TITLE><?php echo "$LDPatientlist $title" ?></TITLE>
...
<b><?php echo "$LDPatientList $title" ?></b>
<ul>
...
function drawdata(&$data)
{
	...
	echo '
	<tr bgcolor="';
	if($toggle) { echo '#e0e0e0">';} else {echo '#ffffff">'; };
	$toggle=!$toggle;
	echo '
	<td>';
...
<?php else : ?>
<p>
<img <?php echo createMascot($root_path,'mascot1_r.gif','0','bottom') ?> align="bottom"><?php echo $LDNoPatientList ?> 
