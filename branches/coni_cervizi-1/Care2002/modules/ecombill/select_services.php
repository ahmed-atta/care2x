...
require_once($root_path.'include/inc_front_chain_lang.php');
    ...
    error_reporting(0);
    connect_db(); */   
//Le righe dalla 23 alla 47 sono state aggiunte da noi!!!!
if($patientno) $_SESSION['numero_paziente']=$patientno;
else $patientno=$_SESSION['numero_paziente'];
if($full_en) $_SESSION['full_en']=$full_en;
else $full_en= $_SESSION['full_en'];
if($itemcode) $_SESSION['item_code']=$itemcode;
else $itemcode=$_SESSION['item_code'];
if($service) $_SESSION['service_']=$service;
else $service=$_SESSION['service_'];
$lang='it'; //perchè cambiava lingua a cavolo
if($sid) $SESSION['sid_']=$sid;
else $sid=$_SESSION['sid_'];



$query="SELECT insurance_firm_id FROM care_encounter WHERE encounter_nr=". $patientno;
$assicurazione=$db->Execute($query);
$assicurazione=$assicurazione->fetchRow();
if(!$assicurazione['insurance_firm_id'])  $assicurazione['insurance_firm_id']=0;
$_SESSION['assicurazione']=$assicurazione['insurance_firm_id'];

 if(($service=='HS')&& ($ricerca=="##########")) $qryLT="SELECT * FROM prezzi_".$assicurazione['insurance_firm_id'] ." WHERE item_type='HS' ";
else if($service=='HS') $qryLT="SELECT * FROM prezzi_".$assicurazione['insurance_firm_id'] ." WHERE item_type='HS' AND item_description LIKE '%".$ricerca."%'";

else if(($service=='LT')&& ($ricerca=="##########")) $qryLT="SELECT * FROM prezzi_".$assicurazione['insurance_firm_id'] ." WHERE item_type='LT' ";
else $qryLT="SELECT * FROM prezzi_".$assicurazione['insurance_firm_id'] ." WHERE item_type='LT' AND item_description LIKE '%".$ricerca."%'";
...
<br>
	  <form action="select_services.php?ricerca="ricerca  >
  <p align = "center">
...
<?php
	...
	echo "<th align=\"center\" height=\"7\" width=\"1050\" bgcolor=\"#CCCCCC\">$TestName</th>";
	echo "<th height=\"7\" width=\"120\" align=\"center\" bgcolor=\"#CCCCCC\">$TestCode</th>";
	echo "<th height=\"7\" width=\"150\" align=\"center\" bgcolor=\"#CCCCCC\">$Costperunit</th>";
	echo "<th height=\"7\" width=\"150\" align=\"center\" valign=\"middle\" bgcolor=\"#CCCCCC\">$NumberofUnits</th>";
	if($cntLT){
	for($cnt=0;$cnt<$cntLT;$cnt++)
	{
		...
		echo "<td align=\"left\" height=\"7\" width=\"1050\">".$item['item_description'];
		echo "</td>";
		echo "<td height=\"7\" width=\"120\" align=\"center\">".$item['item_code'];
		echo "</td>";
		echo "<td height=\"7\" width=\"150\" align=\"center\">".$item['item_unit_cost'];
		echo "</td>";
		echo "<td height=\"7\" width=\"150\" align=\"center\" valign=\"middle\"><select size=\"1\" name=nounits.$cnt id=nounits.$cnt>";
