...
$sql="select * from care_appointment  where (appt_status='Fatto' OR appt_status='In attesa di referto') and (purpose like '%CO419%' OR purpose like '%CO420%') and to_dept_nr=5 and to_dept_id  !='no'";
...
$gg_periodo = 7 ; //una settimana 
	...
	$sql2="select * from care_appointment where pid=".$ris['pid'].
		  ...
		  " and to_dept_nr='5' and purpose not like '%CO419%' and purpose not like '%CO420%'";
...
##########GIORGIO
	...
	{
	$esame=split("#",$ris2['purpose']);
	#echo "CICCIO ".$esame[1]."<br>";
		if ($mappa[$esame[1]][4]==5 && ($esame[1]!='CO419') && ($esame[1]!='CO420'))
		{
	?>
		<tr ><td >
		<?
