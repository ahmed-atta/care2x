...
echo '<p align="center" >';
?><big><big><big><?php echo $mappa['CO420'][0];?></big></big></big>
<?php
echo "</p>";
...
while($risultato=$risulta->FetchRow())
{
  ...
  if($array2[0]=='item_code' && $array2[1]=='CO419')
  {
  $fine=1;
  break;
  }
...
?>
<br /><br /><br />
<big><big><?php echo "Sezione Visista specialistica";?></big></big>
<table border=1>
<tr >
...
<form name='visita_cardiologica_con_ecg_di_base' method="POST">
...
echo "Anamnesi personale negativa per patologia e sintomi di natura cardiovascolare. L'atleta nega di assumere farmaci.";?></textarea>
<br /><br />
</tr>
...
<textarea name="conclusioni_visita_specialistica" cols="139" rows="5"><? if($info['conclusioni_visita_specialistica']) 
echo $info['conclusioni_visita_specialistica'];
else
...
-->
<br /><br />
<big><big><?php echo "Sezione ECG di base";?></big></big>
<table cols="4" rows="2" border="1" cellspacing="10">
...
for($i=0;$i<4;$i++)
{
	echo "<td>";
		...
		{
		if ($info[rtrim($colo[0])])
		echo '<input type="text" size="20" value ="'.$info[rtrim($colo[0])] .'" name="'.rtrim($colo[0]).'">';
		else
		echo '<input type="text" size="20" value ="sinusale" name="'.rtrim($colo[0]).'">';
		}
		echo"</p>";
...
<tr>
<big>Conclusioni:</big><br>
<textarea name="conclusioni_ecg_di_base" cols="120" rows="5"><? if ($info['conclusioni_ecg_di_base'])
echo $info['conclusioni_ecg_di_base'];
else
echo "ECG base nei limiti della norma";?></textarea>
<br /><br />
</tr>
