...
?>modules/laboratory/labor_test_request_printpop.php?sid=<?php   echo $sid ?>&lang=<?php   echo $lang ?>&user_origin=<?php   echo $user_origin ?>&subtarget=chemlabor&batch_nr=<?php   echo $batch_nr ?>&pn=<?php   echo $pn;
?>";
...
<?php   if ($pn == "")
	echo "document.searchform.searchkey.focus();" ?>" 
...
?>" >
<FONT  COLOR="<?php   echo $cfg['top_txtcolor'];
?>"  SIZE=+2  FACE="Arial"><STRONG><?php   echo "$LDDiagnosticTest :: $LDCentralLab";
;
?> </STRONG></FONT>
</td>
<td bgcolor="<?php   echo $cfg['top_bgcolor'];
...
if ($user_origin == 'lab' && $pn) {
	?>
		<a href="<?php   echo $thisfile."?sid=$sid&lang=$lang&station=$station&user_origin=$user_origin&status=$status&target=$target&noresize=$noresize";
	?>"><img <?php   echo createLDImgSrc($root_path, 'newpat2.gif', '0') ?> <?php   if ($cfg['dhtml'])
...
<a href="javascript:gethelp('request_chemlab.php','<?php   echo $pn ?>')"><img <?php   echo createLDImgSrc($root_path, 'hilfe-r.gif', '0') ?>  <?php   if ($cfg['dhtml'])
	echo 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
?>></a><a href="<?php   echo $breakfile ?>" ><img <?php   echo createLDImgSrc($root_path, 'close2.gif', '0') ?>  <?php   if ($cfg['dhtml'])
	echo 'style=filter:alpha(opacity=70) onMouseover=hilite(this,1) onMouseOut=hilite(this,0)';
?>></a></nobr></td>
</tr>
<tr>
...
?><br>
	   ...
	   {*/
?>
<!--	 NON NOSTRO QUESTO COMMENTO HTML sotto -->
<!--
...
-->
   <!-- Day row  -->
<!-- COMMENTATO DA NOI!!!!FINO A RIGA 527
   <tr align="center">
   <?php  
	for ($i = 1; $i < 8; $i ++)
...
-->
<!-- COMMENTATO DA NOI!!!!!!!!!!FINO A RIGA 637
   <tr align="center">
   <?php  
   /* 
...
-->
   <!-- 10, 20 Time row -->
<!-- COMMENTATO DA NOI!!!!!!!!!!!!FINO A RIGA 645
     <tr align="center">
...
<!--COMMENTATO DA NOI!!!! FINO A RIGA 771
      <tr align="center">
...
<!--  Block for the casenumber codes -->  
 <table border=0 cellspacing=0 cellpadding=0>
<?php  
//COMMENTATO DA NOI IL PHP CHE SEGUE!!!!!
/*
for($n=0;$n<8;$n++)
{
...
<!-- COMMENTATA DA NOI LA INTERA FORM CHE CI ERA PER LA SELEZIONE DELLE VARIE ANALISI -->
<!--	
...
while (list ($x, $v) = each($LD_Elements)) {
	...
	if (eregi("tx_", $x)) {
		...
		if (eregi("_x_", $x)) /* Check if the element has two marker fields */ {
			...
			echo '
									         <td '.$tdbgcolor.'>';
			if ($edit)
				echo '<a href="javascript:setM(\''.$elem_index[0].'\')">';
			if ($edit_form || $read_form) {
			...
			if ($edit)
				echo '</a><input type="hidden" name="'.$elem_index[0].'" value="'.$inp_v.'">';

			/* The second marker field on the right */
			echo $v.'</td>
									         <td align="right" '.$tdbgcolor.'>';
			if ($edit)
				echo '<a href="javascript:setM(\''.$elem_index[1].'\')">';
			...
			if ($edit)
				echo '</a><input type="hidden" name="'.$elem_index[1].'" value="'.$inp_v.'">';
			echo '</td>';
		} else {
			...
			if ($edit)
				echo '</a><input type="hidden" name="'.$x.'" value="'.$inp_v.'">';
			echo $v.'</td>';
	...
	if ($tdcount == 6) {
		...
		for ($i = 0; $i < 6; $i ++)
			echo '<td bgcolor="#ffcccc" colspan=2 width=104><img src="p.gif"  width=1 height=1></td><td width=2></td>';
...
?>
  <tr>
    <td colspan=9><input type="text" name="doctor_sign" size=40 maxlength=40 value="<?php   if ($edit_form || $read_form)
	echo stripslashes($stored_request['doctor_sign']);
?>"></td>
    <td colspan=11><input type="text" name="notes" size=65 maxlength=60 value="<?php   if ($edit_form || $read_form)
	echo stripslashes($stored_request['notes']);
?>"></td>
  </tr>
  <tr>
