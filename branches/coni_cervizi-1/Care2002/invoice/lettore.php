...
#echo $appoggio[1];
$appoggio[1]=str_replace("\""," ",$appoggio[1]);
#echo "<br>".$appoggio[1];
 $separatore=strpos(ltrim(rtrim($appoggio[1]))," ");
 #echo "<br>".$separatore;
$query="SELECT * FROM care_person WHERE name_first='".substr((ltrim(rtrim($appoggio[1]))),$separatore+1)."'  AND name_last='".substr(ltrim(rtrim($appoggio[1])),0,$separatore)."'  AND date_birth='".substr($appoggio[2],0,4)."-".substr($appoggio[2],4,2)."-".substr($appoggio[2],6,2)."'";

$appo=mysql_query($query);
$row=mysql_fetch_row($appo);
$RIUSCITA=$row;
...
if($RIUSCITA>0)
echo "ESISTE GIA' <b>".$appoggio[1]."</b><br><br>";
else
{
echo "<i>NON ESISTE <b>".$appoggio[1]."</b>".$query."</i><br><br>";
$query23="SELECT * FROM care_person WHERE name_first='".substr((ltrim(rtrim($appoggio[1]))),$separatore+1)."'  AND name_last='".substr(ltrim(rtrim($appoggio[1])),0,$separatore)."'";
//$prova=$db->Execute($query);
$appo=mysql_query($query23);
$row=mysql_fetch_row($appo);
$RIUSCITA2=$row;
if($RIUSCITA2>0)
{
echo "<br>GUARDA CHE ESISTE GIA' <b>".$appoggio[1]."</b>***************<br><br>";
$contatore++;
}
else
{echo "<i>IN EFFETTI NON ESISTE <b>".$appoggio[1]."</b>".$query."</i><br><br>";
	$data_oggi=date('YmdHis');
	$data_registrazione=date('Y-m-d H:i:s');
	$nome=strtoupper(substr(substr(ltrim(rtrim($appoggio[1])),0,$separatore),0,1)).strtolower(substr(substr(ltrim(rtrim($appoggio[1])),0,$separatore),1));
	$cognome=strtoupper(substr(substr(ltrim(rtrim($appoggio[1])),$separatore+1),0,1)).strtolower(substr(substr(ltrim(rtrim($appoggio[1])),$separatore+1),1));
$query_inserimento="INSERT INTO care_person (date_reg,name_last,name_first,date_birth,name_middle,sss_nr,sex,phone_1_nr,create_time,create_id) VALUES ('".$data_registrazione."','".$nome."','".$cognome."','".substr($appoggio[2],0,4)."-".substr($appoggio[2],4,2)."-".substr($appoggio[2],6,2)."','".strtoupper(substr(trim(str_replace("\""," ",$appoggio[3])),0,1)).strtolower(substr(trim(str_replace("\""," ",$appoggio[3])),1))."','".strtoupper(substr(trim(str_replace("\""," ",$appoggio[4])),0,1)).strtolower(substr(trim(str_replace("\""," ",$appoggio[4])),1))."',".strtolower($appoggio[5]).",'".trim(str_replace("\""," ",$appoggio[6]))."',$data_oggi,'Script Generated')";
echo "<i><b><font size='20'>".$query_inserimento."</font></b></i>";
$db->Execute($query_inserimento);
}
}
...
echo substr($appoggio[2],6,2)."-".substr($appoggio[2],4,2)."-".substr($appoggio[2],0,4)."<br>";#DATA DI NASCITA
echo $appoggio[1]."<br>";
echo "<b>COMUNE DI NASCITA</b> ".$appoggio[3]."<br>";
echo "<b>COMUNE DI RESIDENZA</b> ".$appoggio[4]."<br>";
$appoggio[5]=str_replace("\"","",ltrim(rtrim($appoggio[5])));
echo "<b>SESSO</b> ".$appoggio[5]."<br>";
if ($appoggio[5]!="M"){
	if( $appoggio[5]!="F"){ 
		if( $appoggio[5]!="")
		{echo "<b>ERRORE**********************************************************************</b>";
...
echo "<b>TELEFONO</b>".$appoggio[6]."<br>";

echo "<b>INDIRIZZO</b>".$appoggio[7]."<br>";
echo $appoggio[8]."<br>";
echo $appoggio[9]."<br>";
echo $appoggio[10]."<br>";
echo "<br><br><br>";
